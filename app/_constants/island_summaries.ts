type IslandSummary = {
  uid: string;
  name: string;
  kana: string;
  en_name: string;
  lat: number;
  lng: number;
  prefecture: string;
  city: string;
};

export const islandSummaries: IslandSummary[] = [
  {
    uid: "HKD_RBN_REBUNTO",
    name: "礼文島",
    kana: "れぶんとう",
    en_name: "Rebunto",
    lat: 45.392277,
    lng: 141.015036,
    prefecture: "北海道",
    city: "礼文町",
  },
  {
    uid: "HKD__RISHIRITO",
    name: "利尻島",
    kana: "りしりとう",
    en_name: "Rishirito",
    lat: 45.208568,
    lng: 141.221743,
    prefecture: "北海道",
    city: "利尻町・利尻富士町",
  },
  {
    uid: "HKD_HBR_YAGISHIRITO",
    name: "焼尻島",
    kana: "やぎしりとう",
    en_name: "Yagishirito",
    lat: 44.436402,
    lng: 141.411856,
    prefecture: "北海道",
    city: "羽幌町",
  },
  {
    uid: "HKD_HBR_TEURITO",
    name: "天売島",
    kana: "てうりとう",
    en_name: "Teurito",
    lat: 44.426062,
    lng: 141.318572,
    prefecture: "北海道",
    city: "羽幌町",
  },
  {
    uid: "HKD_OKS_OKUSHIRITO",
    name: "奥尻島",
    kana: "おくしりとう",
    en_name: "Okushirito",
    lat: 42.161276,
    lng: 139.452197,
    prefecture: "北海道",
    city: "奥尻町",
  },
  {
    uid: "HKD_ATG_KOJIMA",
    name: "小島",
    kana: "こじま",
    en_name: "Kojima",
    lat: 42.974718,
    lng: 144.879174,
    prefecture: "北海道",
    city: "厚岸町",
  },
  {
    uid: "MYG_KSN_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 38.85896,
    lng: 141.61563,
    prefecture: "宮城県",
    city: "気仙沼市",
  },
  {
    uid: "MYG_ONG_IZUSHIMA",
    name: "出島",
    kana: "いずしま",
    en_name: "Izushima",
    lat: 38.450341,
    lng: 141.524332,
    prefecture: "宮城県",
    city: "女川町",
  },
  {
    uid: "MYG_ONG_ENOSHIMA",
    name: "江島",
    kana: "えのしま",
    en_name: "Enoshima",
    lat: 38.402912,
    lng: 141.582214,
    prefecture: "宮城県",
    city: "女川町",
  },
  {
    uid: "MYG_ISM_AJISHIMA",
    name: "網地島",
    kana: "あじしま",
    en_name: "Ajishima",
    lat: 38.267618,
    lng: 141.477294,
    prefecture: "宮城県",
    city: "石巻市",
  },
  {
    uid: "MYG_ISM_TASHIROJIMA",
    name: "田代島",
    kana: "たしろじま",
    en_name: "Tashirojima",
    lat: 38.294207,
    lng: 141.416446,
    prefecture: "宮城県",
    city: "石巻市",
  },
  {
    uid: "MYG_SOG_SABUSAWAJIMA",
    name: "寒風沢島",
    kana: "さぶさわじま",
    en_name: "Sabusawajima",
    lat: 38.332662,
    lng: 141.125286,
    prefecture: "宮城県",
    city: "塩竈市",
  },
  {
    uid: "MYG_SOG_NONOSHIMA",
    name: "野々島",
    kana: "ののしま",
    en_name: "Nonoshima",
    lat: 38.339502,
    lng: 141.116532,
    prefecture: "宮城県",
    city: "塩竈市",
  },
  {
    uid: "MYG_SOG_KATSURAJIMA",
    name: "桂島",
    kana: "かつらじま",
    en_name: "Katsurajima",
    lat: 38.334092,
    lng: 141.088068,
    prefecture: "宮城県",
    city: "塩竈市",
  },
  {
    uid: "MYG_SOG_HOOJIMA",
    name: "朴島",
    kana: "ほおじま",
    en_name: "Hoojima",
    lat: 38.351107,
    lng: 141.124119,
    prefecture: "宮城県",
    city: "塩竈市",
  },
  {
    uid: "MYG_ISM_KINKASAN",
    name: "金華山",
    kana: "きんかさん",
    en_name: "Kinkasan",
    lat: 38.297131,
    lng: 141.571343,
    prefecture: "宮城県",
    city: "石巻市",
  },
  {
    uid: "MYG_HMS_MIYATOJIMA",
    name: "宮戸島",
    kana: "みやとじま",
    en_name: "Miyatojima",
    lat: 38.337985,
    lng: 141.15347,
    prefecture: "宮城県",
    city: "東松島市",
  },
  {
    uid: "YMGT_SKT_TOBISHIMA",
    name: "飛島",
    kana: "とびしま",
    en_name: "Tobishima",
    lat: 39.193875,
    lng: 139.54874,
    prefecture: "山形県",
    city: "酒田市",
  },
  {
    uid: "TKY_OSM_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 34.740884,
    lng: 139.385626,
    prefecture: "東京都",
    city: "大島町",
  },
  {
    uid: "TKY_TSM_TOSHIMA",
    name: "利島",
    kana: "としま",
    en_name: "Toshima",
    lat: 34.523601,
    lng: 139.276539,
    prefecture: "東京都",
    city: "利島村",
  },
  {
    uid: "TKY_NJM_NIIJIMA",
    name: "新島",
    kana: "にいじま",
    en_name: "Niijima",
    lat: 34.374649,
    lng: 139.268936,
    prefecture: "東京都",
    city: "新島村",
  },
  {
    uid: "TKY_NJM_SHIKINEJIMA",
    name: "式根島",
    kana: "しきねじま",
    en_name: "Shikinejima",
    lat: 34.325765,
    lng: 139.210679,
    prefecture: "東京都",
    city: "新島村",
  },
  {
    uid: "TKY_KDS_KOZUSHIMA",
    name: "神津島",
    kana: "こうづしま",
    en_name: "Kozushima",
    lat: 34.214573,
    lng: 139.147424,
    prefecture: "東京都",
    city: "神津島村",
  },
  {
    uid: "TKY_MYK_MIYAKEJIMA",
    name: "三宅島",
    kana: "みやけじま",
    en_name: "Miyakejima",
    lat: 34.078842,
    lng: 139.517819,
    prefecture: "東京都",
    city: "三宅村",
  },
  {
    uid: "TKY_MKR_MIKURASHIMA",
    name: "御蔵島",
    kana: "みくらしま",
    en_name: "Mikurashima",
    lat: 33.875053,
    lng: 139.604414,
    prefecture: "東京都",
    city: "御蔵島村",
  },
  {
    uid: "TKY_HCJ_HACHIJOSHIMA",
    name: "八丈島",
    kana: "はちじょうしま",
    en_name: "Hachijoshima",
    lat: 33.103021,
    lng: 139.803557,
    prefecture: "東京都",
    city: "八丈町",
  },
  {
    uid: "TKY_AOG_AOGASHIMA",
    name: "青ケ島",
    kana: "あおがしま",
    en_name: "Aogashima",
    lat: 32.456994,
    lng: 139.765208,
    prefecture: "東京都",
    city: "青ケ島村",
  },
  {
    uid: "TKY_OGS_CHICHIJIMA",
    name: "父島",
    kana: "ちちじま",
    en_name: "Chichijima",
    lat: 27.074353,
    lng: 142.217782,
    prefecture: "東京都",
    city: "小笠原村",
  },
  {
    uid: "TKY_OGS_HAHAJIMA",
    name: "母島",
    kana: "ははじま",
    en_name: "Hahajima",
    lat: 26.6735,
    lng: 142.154075,
    prefecture: "東京都",
    city: "小笠原村",
  },
  {
    uid: "TKY_OGS_IOTO",
    name: "硫黄島",
    kana: "いおうとう",
    en_name: "Ioto",
    lat: 24.787573,
    lng: 141.315565,
    prefecture: "東京都",
    city: "小笠原村",
  },
  {
    uid: "TKY_OGS_MINAMITORISHIMA",
    name: "南鳥島",
    kana: "みなみとりしま",
    en_name: "Minamitorishima",
    lat: 24.286783,
    lng: 153.980777,
    prefecture: "東京都",
    city: "小笠原村",
  },
  {
    uid: "CB_KMG_NIEMONJIMA",
    name: "仁右衛門島",
    kana: "にえもんじま",
    en_name: "Niemonjima",
    lat: 35.076669,
    lng: 140.104231,
    prefecture: "千葉県",
    city: "鴨川市",
  },
  {
    uid: "KNG_MUR_JOGASHIMA",
    name: "城ヶ島",
    kana: "じょうがしま",
    en_name: "Jogashima",
    lat: 35.13426,
    lng: 139.620984,
    prefecture: "神奈川県",
    city: "三浦市",
  },
  {
    uid: "KNG_FJS_ENOSHIMA",
    name: "江の島",
    kana: "えのしま",
    en_name: "Enoshima",
    lat: 35.299099,
    lng: 139.480927,
    prefecture: "神奈川県",
    city: "藤沢市",
  },
  {
    uid: "NGT_AWS_AWASHIMA",
    name: "粟島",
    kana: "あわしま",
    en_name: "Awashima",
    lat: 38.463793,
    lng: 139.238218,
    prefecture: "新潟県",
    city: "粟島浦村",
  },
  {
    uid: "NGT_SD_SADOGASHIMA",
    name: "佐渡島",
    kana: "さどがしま",
    en_name: "Sadogashima",
    lat: 38.041082,
    lng: 138.338718,
    prefecture: "新潟県",
    city: "佐渡市",
  },
  {
    uid: "ISK_WJM_HEGURAJIMA",
    name: "舳倉島",
    kana: "へぐらじま",
    en_name: "Hegurajima",
    lat: 37.851961,
    lng: 136.919766,
    prefecture: "石川県",
    city: "輪島市",
  },
  {
    uid: "ISK_NNO_NOTOJIMA",
    name: "能登島",
    kana: "のとじま",
    en_name: "Notojima",
    lat: 37.131244,
    lng: 136.96624,
    prefecture: "石川県",
    city: "七尾市",
  },
  {
    uid: "SZO_ATM_HATSUSHIMA",
    name: "初島",
    kana: "はつしま",
    en_name: "Hatsushima",
    lat: 35.040517,
    lng: 139.171453,
    prefecture: "静岡県",
    city: "熱海市",
  },
  {
    uid: "AIC_NSO_SAKUSHIMA",
    name: "佐久島",
    kana: "さくしま",
    en_name: "Sakushima",
    lat: 34.728985,
    lng: 137.040059,
    prefecture: "愛知県",
    city: "西尾市",
  },
  {
    uid: "AIC_MNM_HIMAKAJIMA",
    name: "日間賀島",
    kana: "ひまかじま",
    en_name: "Himakajima",
    lat: 34.705104,
    lng: 137.006023,
    prefecture: "愛知県",
    city: "南知多町",
  },
  {
    uid: "AIC_MNM_SHINOJIMA",
    name: "篠島",
    kana: "しのじま",
    en_name: "Shinojima",
    lat: 34.679059,
    lng: 137.008453,
    prefecture: "愛知県",
    city: "南知多町",
  },
  {
    uid: "ME_TB_KAMISHIMA",
    name: "神島",
    kana: "かみしま",
    en_name: "Kamishima",
    lat: 34.547603,
    lng: 136.981725,
    prefecture: "三重県",
    city: "鳥羽市",
  },
  {
    uid: "ME_TB_TOUSHIJIMA",
    name: "答志島",
    kana: "とうしじま",
    en_name: "Toushijima",
    lat: 34.520595,
    lng: 136.881013,
    prefecture: "三重県",
    city: "鳥羽市",
  },
  {
    uid: "ME_TB_SUGASHIMA",
    name: "菅島",
    kana: "すがしま",
    en_name: "Sugashima",
    lat: 34.488667,
    lng: 136.890712,
    prefecture: "三重県",
    city: "鳥羽市",
  },
  {
    uid: "ME_TB_SAKATEJIMA",
    name: "坂手島",
    kana: "さかてじま",
    en_name: "Sakatejima",
    lat: 34.487395,
    lng: 136.856776,
    prefecture: "三重県",
    city: "鳥羽市",
  },
  {
    uid: "ME_SM_WATAKANOJIMA",
    name: "渡鹿野島",
    kana: "わたかのじま",
    en_name: "Watakanojima",
    lat: 34.363025,
    lng: 136.874953,
    prefecture: "三重県",
    city: "志摩市",
  },
  {
    uid: "ME_SM_MASAKIJIMA",
    name: "間崎島",
    kana: "まさきじま",
    en_name: "Masakijima",
    lat: 34.289717,
    lng: 136.815572,
    prefecture: "三重県",
    city: "志摩市",
  },
  {
    uid: "ME_SM_KASHIKOJIMA",
    name: "賢島",
    kana: "かしこじま",
    en_name: "Kashikojima",
    lat: 34.308435,
    lng: 136.818696,
    prefecture: "三重県",
    city: "志摩市",
  },
  {
    uid: "ME_SM_YOKOYAMAJIMA",
    name: "横山島",
    kana: "よこやまじま",
    en_name: "Yokoyamajima",
    lat: 34.30033,
    lng: 136.820138,
    prefecture: "三重県",
    city: "志摩市",
  },
  {
    uid: "SIG_OUM_OKISHIMA",
    name: "沖島",
    kana: "おきしま",
    en_name: "Okishima",
    lat: 35.207667,
    lng: 136.063869,
    prefecture: "滋賀県",
    city: "近江八幡市",
  },
  {
    uid: "HYG__AWAJISHIMA",
    name: "淡路島",
    kana: "あわじしま",
    en_name: "Awajishima",
    lat: 34.357592,
    lng: 134.816211,
    prefecture: "兵庫県",
    city: "淡路市・洲本市・南あわじ市",
  },
  {
    uid: "HYG_MAW_NUSHIMA",
    name: "沼島",
    kana: "ぬしま",
    en_name: "Nushima",
    lat: 34.166733,
    lng: 134.824814,
    prefecture: "兵庫県",
    city: "南あわじ市",
  },
  {
    uid: "HYG_HMJ_IESHIMA",
    name: "家島",
    kana: "いえしま",
    en_name: "Ieshima",
    lat: 34.668058,
    lng: 134.529002,
    prefecture: "兵庫県",
    city: "姫路市",
  },
  {
    uid: "HYG_HMJ_TANGASHIMA",
    name: "男鹿島",
    kana: "たんがしま",
    en_name: "Tangashima",
    lat: 34.663018,
    lng: 134.573729,
    prefecture: "兵庫県",
    city: "姫路市",
  },
  {
    uid: "HYG_HMJ_BOZEJIMA",
    name: "坊勢島",
    kana: "ぼうぜじま",
    en_name: "Bozejima",
    lat: 34.645024,
    lng: 134.508979,
    prefecture: "兵庫県",
    city: "姫路市",
  },
  {
    uid: "HYG_HMJ_NISHIJIMA",
    name: "西島",
    kana: "にしじま",
    en_name: "Nishijima",
    lat: 34.654143,
    lng: 134.481875,
    prefecture: "兵庫県",
    city: "姫路市",
  },
  {
    uid: "WKY_NCK_NAKANOSHIMA",
    name: "中の島",
    kana: "なかのしま",
    en_name: "Nakanoshima",
    lat: 33.621772,
    lng: 135.94575,
    prefecture: "和歌山県",
    city: "那智勝浦町",
  },
  {
    uid: "WKY_KSM_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 33.469242,
    lng: 135.811365,
    prefecture: "和歌山県",
    city: "串本町",
  },
  {
    uid: "SMN_OKN_DOGO",
    name: "島後",
    kana: "どうご",
    en_name: "Dogo",
    lat: 36.20915,
    lng: 133.321695,
    prefecture: "島根県",
    city: "隠岐の島町",
  },
  {
    uid: "SMN_AM_NAKANOSHIMA",
    name: "中ノ島",
    kana: "なかのしま",
    en_name: "Nakanoshima",
    lat: 36.085744,
    lng: 133.102013,
    prefecture: "島根県",
    city: "海士町",
  },
  {
    uid: "SMN_NSN_NISHINOSHIMA",
    name: "西ノ島",
    kana: "にしのしま",
    en_name: "Nishinoshima",
    lat: 36.083921,
    lng: 132.966021,
    prefecture: "島根県",
    city: "西ノ島町",
  },
  {
    uid: "SMN_CB_CHIBURISHIMA",
    name: "知夫里島",
    kana: "ちぶりしま",
    en_name: "Chiburishima",
    lat: 36.01813,
    lng: 133.034062,
    prefecture: "島根県",
    city: "知夫村",
  },
  {
    uid: "SMN_MTE_DAIKONJIMA",
    name: "大根島",
    kana: "だいこんじま",
    en_name: "Daikonjima",
    lat: 35.494886,
    lng: 133.17113,
    prefecture: "島根県",
    city: "松江市",
  },
  {
    uid: "SMN_MTE_ESHIMA",
    name: "江島",
    kana: "えしま",
    en_name: "Eshima",
    lat: 35.516039,
    lng: 133.192258,
    prefecture: "島根県",
    city: "松江市",
  },
  {
    uid: "OKY_BZN_KAKUIJIMA",
    name: "鹿久居島",
    kana: "かくいじま",
    en_name: "Kakuijima",
    lat: 34.714248,
    lng: 134.311827,
    prefecture: "岡山県",
    city: "備前市",
  },
  {
    uid: "OKY_BZN_OHTABUJIMA",
    name: "大多府島",
    kana: "おおたぶじま",
    en_name: "Ohtabujima",
    lat: 34.683114,
    lng: 134.295854,
    prefecture: "岡山県",
    city: "備前市",
  },
  {
    uid: "OKY_BZN_KASHIRAJIMA",
    name: "頭島",
    kana: "かしらじま",
    en_name: "Kashirajima",
    lat: 34.699979,
    lng: 134.286977,
    prefecture: "岡山県",
    city: "備前市",
  },
  {
    uid: "OKY_BZN_KOUJIMA",
    name: "鴻島",
    kana: "こうじま",
    en_name: "Koujima",
    lat: 34.699574,
    lng: 134.258562,
    prefecture: "岡山県",
    city: "備前市",
  },
  {
    uid: "OKY_STU_NAGASHIMA",
    name: "長島",
    kana: "ながしま",
    en_name: "Nagashima",
    lat: 34.67355,
    lng: 134.232499,
    prefecture: "岡山県",
    city: "瀬戸内市",
  },
  {
    uid: "OKY_STU_MAEJIMA",
    name: "前島",
    kana: "まえじま",
    en_name: "Maejima",
    lat: 34.608096,
    lng: 134.185076,
    prefecture: "岡山県",
    city: "瀬戸内市",
  },
  {
    uid: "OKY_STU_KIJIMA",
    name: "黄島",
    kana: "きじま",
    en_name: "Kijima",
    lat: 34.598492,
    lng: 134.195157,
    prefecture: "岡山県",
    city: "瀬戸内市",
  },
  {
    uid: "OKY_STU_KUROSHIMA",
    name: "黒島",
    kana: "くろしま",
    en_name: "Kuroshima",
    lat: 34.598429,
    lng: 134.159793,
    prefecture: "岡山県",
    city: "瀬戸内市",
  },
  {
    uid: "OKY_OKY_INUJIMA",
    name: "犬島",
    kana: "いぬじま",
    en_name: "Inujima",
    lat: 34.56312,
    lng: 134.101973,
    prefecture: "岡山県",
    city: "岡山市",
  },
  {
    uid: "OKY_TMN_ISHIMA",
    name: "石島",
    kana: "いしま",
    en_name: "Ishima",
    lat: 34.501127,
    lng: 134.016344,
    prefecture: "岡山県",
    city: "玉野市",
  },
  {
    uid: "OKY_KRS_MATSUSHIMA",
    name: "松島",
    kana: "まつしま",
    en_name: "Matsushima",
    lat: 34.425379,
    lng: 133.817439,
    prefecture: "岡山県",
    city: "倉敷市",
  },
  {
    uid: "OKY_KRS_MUGUCHIJIMA",
    name: "六口島",
    kana: "むぐちじま",
    en_name: "Muguchijima",
    lat: 34.424129,
    lng: 133.768118,
    prefecture: "岡山県",
    city: "倉敷市",
  },
  {
    uid: "OKY_KSO_TAKASHIMA",
    name: "高島",
    kana: "たかしま",
    en_name: "Takashima",
    lat: 34.429109,
    lng: 133.509003,
    prefecture: "岡山県",
    city: "笠岡市",
  },
  {
    uid: "OKY_KSO_SHIRAISHIJIMA",
    name: "白石島",
    kana: "しらいしじま",
    en_name: "Shiraishijima",
    lat: 34.400536,
    lng: 133.515016,
    prefecture: "岡山県",
    city: "笠岡市",
  },
  {
    uid: "OKY_KSO_KITAGISHIMA",
    name: "北木島",
    kana: "きたぎしま",
    en_name: "Kitagishima",
    lat: 34.379688,
    lng: 133.536658,
    prefecture: "岡山県",
    city: "笠岡市",
  },
  {
    uid: "OKY_KSO_MANABESHIMA",
    name: "真鍋島",
    kana: "まなべしま",
    en_name: "Manabeshima",
    lat: 34.352451,
    lng: 133.572708,
    prefecture: "岡山県",
    city: "笠岡市",
  },
  {
    uid: "OKY_KSO_KOBISHIMA",
    name: "小飛島",
    kana: "こびしま",
    en_name: "Kobishima",
    lat: 34.347162,
    lng: 133.521029,
    prefecture: "岡山県",
    city: "笠岡市",
  },
  {
    uid: "OKY_KSO_OHBISHIMA",
    name: "大飛島",
    kana: "おおびしま",
    en_name: "Ohbishima",
    lat: 34.341676,
    lng: 133.505394,
    prefecture: "岡山県",
    city: "笠岡市",
  },
  {
    uid: "OKY_KSO_MUSHIMA",
    name: "六島",
    kana: "むしま",
    en_name: "Mushima",
    lat: 34.303512,
    lng: 133.533052,
    prefecture: "岡山県",
    city: "笠岡市",
  },
  {
    uid: "HRS_FKY_HASIRIJIMA",
    name: "走島",
    kana: "はしりじま",
    en_name: "Hasirijima",
    lat: 34.344373,
    lng: 133.437988,
    prefecture: "広島県",
    city: "福山市",
  },
  {
    uid: "HRS_FKY_TASHIMA",
    name: "田島",
    kana: "たしま",
    en_name: "Tashima",
    lat: 34.360138,
    lng: 133.314978,
    prefecture: "広島県",
    city: "福山市",
  },
  {
    uid: "HRS_FKY_YOKOSHIMA",
    name: "横島",
    kana: "よこしま",
    en_name: "Yokoshima",
    lat: 34.34427,
    lng: 133.273908,
    prefecture: "広島県",
    city: "福山市",
  },
  {
    uid: "HRS_ONM_MOMOSHIMA",
    name: "百島",
    kana: "ももしま",
    en_name: "Momoshima",
    lat: 34.372311,
    lng: 133.266656,
    prefecture: "広島県",
    city: "尾道市",
  },
  {
    uid: "HRS_ONM_MUKAISHIMA",
    name: "向島",
    kana: "むかいしま",
    en_name: "Mukaishima",
    lat: 34.379664,
    lng: 133.208608,
    prefecture: "広島県",
    city: "尾道市",
  },
  {
    uid: "HRS_ONM_IWASHIJIMA",
    name: "岩子島",
    kana: "いわしじま",
    en_name: "Iwashijima",
    lat: 34.379432,
    lng: 133.162606,
    prefecture: "広島県",
    city: "尾道市",
  },
  {
    uid: "HRS_ONM_HOSOJIMA",
    name: "細島",
    kana: "ほそじま",
    en_name: "Hosojima",
    lat: 34.36573,
    lng: 133.139589,
    prefecture: "広島県",
    city: "尾道市",
  },
  {
    uid: "HRS_ONM_INNOSHIMA",
    name: "因島",
    kana: "いんのしま",
    en_name: "Innoshima",
    lat: 34.321804,
    lng: 133.165029,
    prefecture: "広島県",
    city: "尾道市",
  },
  {
    uid: "HRS_MHR_SAGIJIMA",
    name: "佐木島",
    kana: "さぎじま",
    en_name: "Sagijima",
    lat: 34.340544,
    lng: 133.111713,
    prefecture: "広島県",
    city: "三原市",
  },
  {
    uid: "HRS_MHR_KOSAGIJIMA",
    name: "小佐木島",
    kana: "こさぎじま",
    en_name: "Kosagijima",
    lat: 34.363791,
    lng: 133.103832,
    prefecture: "広島県",
    city: "三原市",
  },
  {
    uid: "HRS_ONM_IKUCHIJIMA",
    name: "生口島",
    kana: "いくちじま",
    en_name: "Ikuchijima",
    lat: 34.292832,
    lng: 133.106863,
    prefecture: "広島県",
    city: "尾道市",
  },
  {
    uid: "HRS_ONM_KOUNESHIMA",
    name: "高根島",
    kana: "こうねしま",
    en_name: "Kouneshima",
    lat: 34.312153,
    lng: 133.072902,
    prefecture: "広島県",
    city: "尾道市",
  },
  {
    uid: "HRS_TKH_OHKUNOSHIMA",
    name: "大久野島",
    kana: "おおくのしま",
    en_name: "Ohkunoshima",
    lat: 34.30906,
    lng: 132.993977,
    prefecture: "広島県",
    city: "竹原市",
  },
  {
    uid: "HRS_OSK_CHIGIRIJIMA",
    name: "契島",
    kana: "ちぎりじま",
    en_name: "Chigirijima",
    lat: 34.296821,
    lng: 132.902453,
    prefecture: "広島県",
    city: "大崎上島町",
  },
  {
    uid: "HRS_OSK_IKUNOSHIMA",
    name: "生野島",
    kana: "いくのしま",
    en_name: "Ikunoshima",
    lat: 34.287734,
    lng: 132.91493,
    prefecture: "広島県",
    city: "大崎上島町",
  },
  {
    uid: "HRS_OSK_OHSAKIKAMIJIMA",
    name: "大崎上島",
    kana: "おおさきかみじま",
    en_name: "Ohsakikamijima",
    lat: 34.241898,
    lng: 132.871196,
    prefecture: "広島県",
    city: "大崎上島町",
  },
  {
    uid: "HRS_OSK_NAGASHIMA",
    name: "長島",
    kana: "ながしま",
    en_name: "Nagashima",
    lat: 34.264735,
    lng: 132.86512,
    prefecture: "広島県",
    city: "大崎上島町",
  },
  {
    uid: "HRS_KR_OSAKISHIMOJIMA",
    name: "大崎下島",
    kana: "おおさきしもじま",
    en_name: "Osakishimojima",
    lat: 34.172306,
    lng: 132.829664,
    prefecture: "広島県",
    city: "呉市",
  },
  {
    uid: "HRS_KR_MIKADOJIMA",
    name: "三角島",
    kana: "みかどじま",
    en_name: "Mikadojima",
    lat: 34.192867,
    lng: 132.816253,
    prefecture: "広島県",
    city: "呉市",
  },
  {
    uid: "HRS_KR_TOYOSHIMA",
    name: "豊島",
    kana: "とよしま",
    en_name: "Toyoshima",
    lat: 34.171778,
    lng: 132.783318,
    prefecture: "広島県",
    city: "呉市",
  },
  {
    uid: "HRS_KR_ITSUKIJIMA",
    name: "斎島",
    kana: "いつきしま",
    en_name: "Itsukijima",
    lat: 34.116892,
    lng: 132.791859,
    prefecture: "広島県",
    city: "呉市",
  },
  {
    uid: "HRS_HGS_OHSHIBAJIMA",
    name: "大芝島",
    kana: "おおしばじま",
    en_name: "Ohshibajima",
    lat: 34.274203,
    lng: 132.801618,
    prefecture: "広島県",
    city: "東広島市",
  },
  {
    uid: "HRS_KR_KAMIKAMAGARIJIMA",
    name: "上蒲刈島",
    kana: "かみかまがりじま",
    en_name: "Kamikamagarijima",
    lat: 34.188723,
    lng: 132.717386,
    prefecture: "広島県",
    city: "呉市",
  },
  {
    uid: "HRS_KR_SHIMOKAMAGARIJIMA",
    name: "下蒲刈島",
    kana: "しもかまがりじま",
    en_name: "Shimokamagarijima",
    lat: 34.189011,
    lng: 132.663601,
    prefecture: "広島県",
    city: "呉市",
  },
  {
    uid: "HRS_KR_NASAKEJIMA",
    name: "情島",
    kana: "なさけじま",
    en_name: "Nasakejima",
    lat: 34.162259,
    lng: 132.574243,
    prefecture: "広島県",
    city: "呉市",
  },
  {
    uid: "HRS_HRS_NINOSIMA",
    name: "似島",
    kana: "にのしま",
    en_name: "Ninosima",
    lat: 34.306876,
    lng: 132.435619,
    prefecture: "広島県",
    city: "広島市",
  },
  {
    uid: "HRS_HRS_KANAWAJIMA",
    name: "金輪島",
    kana: "かなわじま",
    en_name: "Kanawajima",
    lat: 34.338813,
    lng: 132.48105,
    prefecture: "広島県",
    city: "広島市",
  },
  {
    uid: "HRS_ETJ_ETAJIMA_NOUMISHIMA",
    name: "江田島・能美島",
    kana: "えたじま・のうみじま",
    en_name: "Etajima/Noumishima",
    lat: 34.256557,
    lng: 132.414088,
    prefecture: "広島県",
    city: "江田島市",
  },
  {
    uid: "HRS_ETJ_OKINOSHIMA",
    name: "沖野島",
    kana: "おきのしま",
    en_name: "Okinoshima",
    lat: 34.153336,
    lng: 132.434391,
    prefecture: "広島県",
    city: "江田島市",
  },
  {
    uid: "HRS_KR_KURAHASHIJIMA",
    name: "倉橋島",
    kana: "くらはしじま",
    en_name: "Kurahashijima",
    lat: 34.126548,
    lng: 132.521536,
    prefecture: "広島県",
    city: "呉市",
  },
  {
    uid: "HRS_KR_KASHIMA",
    name: "鹿島",
    kana: "かしま",
    en_name: "Kashima",
    lat: 34.052627,
    lng: 132.528893,
    prefecture: "広島県",
    city: "呉市",
  },
  {
    uid: "HRS_HTK_ITSUKUSHIMA",
    name: "厳島",
    kana: "いつくしま",
    en_name: "Itsukushima",
    lat: 34.273211,
    lng: 132.305249,
    prefecture: "広島県",
    city: "廿日市市",
  },
  {
    uid: "HRS_OTK_ATATAJIMA",
    name: "阿多田島",
    kana: "あたたじま",
    en_name: "Atatajima",
    lat: 34.192183,
    lng: 132.307712,
    prefecture: "広島県",
    city: "大竹市",
  },
  {
    uid: "YMGC_IWK_HASHIMA",
    name: "端島",
    kana: "はしま",
    en_name: "Hashima",
    lat: 34.032443,
    lng: 132.385231,
    prefecture: "山口県",
    city: "岩国市",
  },
  {
    uid: "YMGC_IWK_HASHIRAJIMA",
    name: "柱島",
    kana: "はしらじま",
    en_name: "Hashirajima",
    lat: 34.018285,
    lng: 132.415961,
    prefecture: "山口県",
    city: "岩国市",
  },
  {
    uid: "YMGC_IWK_KUROSHIMA",
    name: "黒島",
    kana: "くろしま",
    en_name: "Kuroshima",
    lat: 33.996732,
    lng: 132.332228,
    prefecture: "山口県",
    city: "岩国市",
  },
  {
    uid: "YMGC_SOO_YASHIROJIMA",
    name: "屋代島",
    kana: "やしろじま",
    en_name: "Yashirojima",
    lat: 33.910327,
    lng: 132.216332,
    prefecture: "山口県",
    city: "周防大島町",
  },
  {
    uid: "YMGC_SOO_KASASAJIMA",
    name: "笠佐島",
    kana: "かささじま",
    en_name: "Kasasajima",
    lat: 33.94361,
    lng: 132.160975,
    prefecture: "山口県",
    city: "周防大島町",
  },
  {
    uid: "YMGC_SOO_MAEJIMA",
    name: "前島",
    kana: "まえじま",
    en_name: "Maejima",
    lat: 34.003248,
    lng: 132.264596,
    prefecture: "山口県",
    city: "周防大島町",
  },
  {
    uid: "YMGC_SOO_UKASHIMA",
    name: "浮島",
    kana: "うかしま",
    en_name: "Ukashima",
    lat: 33.951265,
    lng: 132.347101,
    prefecture: "山口県",
    city: "周防大島町",
  },
  {
    uid: "YMGC_SOO_NASAKEJIMA",
    name: "情島",
    kana: "なさけじま",
    en_name: "Nasakejima",
    lat: 33.960645,
    lng: 132.473685,
    prefecture: "山口県",
    city: "周防大島町",
  },
  {
    uid: "YMGC_SOO_OKIKAMUROJIMA",
    name: "沖家室島",
    kana: "おきかむろじま",
    en_name: "Okikamurojima",
    lat: 33.853369,
    lng: 132.361864,
    prefecture: "山口県",
    city: "周防大島町",
  },
  {
    uid: "YMGC_YNI_HEIGUNTO",
    name: "平郡島",
    kana: "へいぐんとう",
    en_name: "Heigunto",
    lat: 33.790809,
    lng: 132.221445,
    prefecture: "山口県",
    city: "柳井市",
  },
  {
    uid: "YMGC_KMN_NAGASHIMA",
    name: "長島",
    kana: "ながしま",
    en_name: "Nagashima",
    lat: 33.835378,
    lng: 132.093018,
    prefecture: "山口県",
    city: "上関町",
  },
  {
    uid: "YMGC_KMN_YASHIMA",
    name: "八島",
    kana: "やしま",
    en_name: "Yashima",
    lat: 33.724,
    lng: 132.14492,
    prefecture: "山口県",
    city: "上関町",
  },
  {
    uid: "YMGC_KMN_IWAISHIMA",
    name: "祝島",
    kana: "いわいしま",
    en_name: "Iwaishima",
    lat: 33.783654,
    lng: 131.974195,
    prefecture: "山口県",
    city: "上関町",
  },
  {
    uid: "YMGC_HRO_SAGOUJIMA",
    name: "佐合島",
    kana: "さごうじま",
    en_name: "Sagoujima",
    lat: 33.872032,
    lng: 132.065811,
    prefecture: "山口県",
    city: "平生町",
  },
  {
    uid: "YMGC_TBS_UMASHIMA",
    name: "馬島",
    kana: "うましま",
    en_name: "Umashima",
    lat: 33.893691,
    lng: 132.049728,
    prefecture: "山口県",
    city: "田布施町",
  },
  {
    uid: "YMGC_HKR_USHIMA",
    name: "牛島",
    kana: "うしま",
    en_name: "Ushima",
    lat: 33.85589,
    lng: 132.016309,
    prefecture: "山口県",
    city: "光市",
  },
  {
    uid: "YMGC_KDM_KASADOJIMA",
    name: "笠戸島",
    kana: "かさどしま",
    en_name: "Kasadojima",
    lat: 33.948554,
    lng: 131.837721,
    prefecture: "山口県",
    city: "下松市",
  },
  {
    uid: "YMGC_SYN_SUKUMOJIMA",
    name: "粭島",
    kana: "すくもじま",
    en_name: "Sukumojima",
    lat: 33.978818,
    lng: 131.761891,
    prefecture: "山口県",
    city: "周南市",
  },
  {
    uid: "YMGC_SYN_OHDUSHIMA",
    name: "大津島",
    kana: "おおづしま",
    en_name: "Ohdushima",
    lat: 34.00607,
    lng: 131.707129,
    prefecture: "山口県",
    city: "周南市",
  },
  {
    uid: "YMGC_HUF_NOSHIMA",
    name: "野島",
    kana: "のしま",
    en_name: "Noshima",
    lat: 33.94334,
    lng: 131.697166,
    prefecture: "山口県",
    city: "防府市",
  },
  {
    uid: "YMGC_HUF_MUKOJIMA",
    name: "向島",
    kana: "むこうじま",
    en_name: "Mukojima",
    lat: 34.003544,
    lng: 131.576223,
    prefecture: "山口県",
    city: "防府市",
  },
  {
    uid: "YMGC_SMN_TAKENOKOJIMA",
    name: "竹ノ子島",
    kana: "たけのこじま",
    en_name: "Takenokojima",
    lat: 33.952228,
    lng: 130.877227,
    prefecture: "山口県",
    city: "下関市",
  },
  {
    uid: "YMGC_SMN_HIKOSHIMA",
    name: "彦島",
    kana: "ひこしま",
    en_name: "Hikoshima",
    lat: 33.918759,
    lng: 130.91349,
    prefecture: "山口県",
    city: "下関市",
  },
  {
    uid: "YMGC_HG_MISHIMA",
    name: "見島",
    kana: "みしま",
    en_name: "Mishima",
    lat: 34.770023,
    lng: 131.145167,
    prefecture: "山口県",
    city: "萩市",
  },
  {
    uid: "YMGC_HG_AISHIMA",
    name: "相島",
    kana: "あいしま",
    en_name: "Aishima",
    lat: 34.508671,
    lng: 131.283367,
    prefecture: "山口県",
    city: "萩市",
  },
  {
    uid: "YMGC_HG_HITSUSHIMA",
    name: "櫃島",
    kana: "ひつしま",
    en_name: "Hitsushima",
    lat: 34.517708,
    lng: 131.38742,
    prefecture: "山口県",
    city: "萩市",
  },
  {
    uid: "YMGC_HG_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 34.498599,
    lng: 131.408709,
    prefecture: "山口県",
    city: "萩市",
  },
  {
    uid: "YMGC_NGT_OHMIJIMA",
    name: "青海島",
    kana: "おおみじま",
    en_name: "Ohmijima",
    lat: 34.428957,
    lng: 131.222167,
    prefecture: "山口県",
    city: "長門市",
  },
  {
    uid: "YMGC_SMN_TSUNOSHIMA",
    name: "角島",
    kana: "つのしま",
    en_name: "Tsunoshima",
    lat: 34.360624,
    lng: 130.875331,
    prefecture: "山口県",
    city: "下関市",
  },
  {
    uid: "YMGC_SMN_FUTAOIJIMA",
    name: "蓋井島",
    kana: "ふたおいじま",
    en_name: "Futaoijima",
    lat: 34.107102,
    lng: 130.779193,
    prefecture: "山口県",
    city: "下関市",
  },
  {
    uid: "YMGC_SMN_MUTSUREJIMA",
    name: "六連島",
    kana: "むつれじま",
    en_name: "Mutsurejima",
    lat: 33.974398,
    lng: 130.863955,
    prefecture: "山口県",
    city: "下関市",
  },
  {
    uid: "TKS_NRT_OHGESHIMA",
    name: "大毛島",
    kana: "おおげしま",
    en_name: "Ohgeshima",
    lat: 34.196004,
    lng: 134.620763,
    prefecture: "徳島県",
    city: "鳴門市",
  },
  {
    uid: "TKS_NRT_SHIMADAJIMA",
    name: "島田島",
    kana: "しまだじま",
    en_name: "Shimadajima",
    lat: 34.237023,
    lng: 134.604307,
    prefecture: "徳島県",
    city: "鳴門市",
  },
  {
    uid: "TKS_ANN_ISHIMA",
    name: "伊島",
    kana: "いしま",
    en_name: "Ishima",
    lat: 33.847003,
    lng: 134.814281,
    prefecture: "徳島県",
    city: "阿南市",
  },
  {
    uid: "TKS_MG_TEBAJIMA",
    name: "出羽島",
    kana: "てばじま",
    en_name: "Tebajima",
    lat: 33.632613,
    lng: 134.42408,
    prefecture: "徳島県",
    city: "牟岐町",
  },
  {
    uid: "TKS_KIY_TAKEGASHIMA",
    name: "竹ヶ島",
    kana: "たけがしま",
    en_name: "Takegashima",
    lat: 33.543269,
    lng: 134.317742,
    prefecture: "徳島県",
    city: "海陽町",
  },
  {
    uid: "KGW__SHODOSHIMA",
    name: "小豆島",
    kana: "しょうどしま",
    en_name: "Shodoshima",
    lat: 34.514431,
    lng: 134.303542,
    prefecture: "香川県",
    city: "小豆島町・土庄町",
  },
  {
    uid: "KGW_TNS_OKINOSHIMA",
    name: "沖之島",
    kana: "おきのしま",
    en_name: "Okinoshima",
    lat: 34.519997,
    lng: 134.161331,
    prefecture: "香川県",
    city: "土庄町",
  },
  {
    uid: "KGW_TNS_ODESHIMA",
    name: "小豊島",
    kana: "おでしま",
    en_name: "Odeshima",
    lat: 34.47673,
    lng: 134.117417,
    prefecture: "香川県",
    city: "土庄町",
  },
  {
    uid: "KGW_TNS_TESHIMA",
    name: "豊島",
    kana: "てしま",
    en_name: "Teshima",
    lat: 34.481483,
    lng: 134.072258,
    prefecture: "香川県",
    city: "土庄町",
  },
  {
    uid: "KGW_NOS_NAOSHIMA",
    name: "直島",
    kana: "なおしま",
    en_name: "Naoshima",
    lat: 34.45978,
    lng: 133.995641,
    prefecture: "香川県",
    city: "直島町",
  },
  {
    uid: "KGW_NOS_MUKAEJIMA",
    name: "向島",
    kana: "むかえじま",
    en_name: "Mukaejima",
    lat: 34.467251,
    lng: 134.002058,
    prefecture: "香川県",
    city: "直島町",
  },
  {
    uid: "KGW_NOS_BYOBUSHIMA",
    name: "屏風島",
    kana: "びょうぶしま",
    en_name: "Byobushima",
    lat: 34.499416,
    lng: 133.971686,
    prefecture: "香川県",
    city: "直島町",
  },
  {
    uid: "KGW_TKM_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 34.406582,
    lng: 134.106725,
    prefecture: "香川県",
    city: "高松市",
  },
  {
    uid: "KGW_TKM_OGIJIMA",
    name: "男木島",
    kana: "おぎじま",
    en_name: "Ogijima",
    lat: 34.426397,
    lng: 134.060367,
    prefecture: "香川県",
    city: "高松市",
  },
  {
    uid: "KGW_TKM_MEGIJIMA",
    name: "女木島",
    kana: "めぎじま",
    en_name: "Megijima",
    lat: 34.394843,
    lng: 134.050852,
    prefecture: "香川県",
    city: "高松市",
  },
  {
    uid: "KGW_SKI_HITSUISHIJIMA",
    name: "櫃石島",
    kana: "ひついしじま",
    en_name: "Hitsuishijima",
    lat: 34.421571,
    lng: 133.803998,
    prefecture: "香川県",
    city: "坂出市",
  },
  {
    uid: "KGW_SKI_IWAKUROJIMA",
    name: "岩黒島",
    kana: "いわくろじま",
    en_name: "Iwakurojima",
    lat: 34.406249,
    lng: 133.809378,
    prefecture: "香川県",
    city: "坂出市",
  },
  {
    uid: "KGW_SKI_YOSHIMA",
    name: "与島",
    kana: "よしま",
    en_name: "Yoshima",
    lat: 34.387611,
    lng: 133.817148,
    prefecture: "香川県",
    city: "坂出市",
  },
  {
    uid: "KGW_SKI_KOYOSHIMA",
    name: "小与島",
    kana: "こよしま",
    en_name: "Koyoshima",
    lat: 34.391839,
    lng: 133.830294,
    prefecture: "香川県",
    city: "坂出市",
  },
  {
    uid: "KGW_MRG_USHIJIMA",
    name: "牛島",
    kana: "うしじま",
    en_name: "Ushijima",
    lat: 34.359763,
    lng: 133.778885,
    prefecture: "香川県",
    city: "丸亀市",
  },
  {
    uid: "KGW_MRG_HONJIMA",
    name: "本島",
    kana: "ほんじま",
    en_name: "Honjima",
    lat: 34.386514,
    lng: 133.769315,
    prefecture: "香川県",
    city: "丸亀市",
  },
  {
    uid: "KGW_MRG_HIROSHIMA",
    name: "広島",
    kana: "ひろしま",
    en_name: "Hiroshima",
    lat: 34.374984,
    lng: 133.709457,
    prefecture: "香川県",
    city: "丸亀市",
  },
  {
    uid: "KGW_MRG_TESHIMA",
    name: "手島",
    kana: "てしま",
    en_name: "Teshima",
    lat: 34.399873,
    lng: 133.668712,
    prefecture: "香川県",
    city: "丸亀市",
  },
  {
    uid: "KGW_MRG_OTESHIMA",
    name: "小手島",
    kana: "おてしま",
    en_name: "Oteshima",
    lat: 34.376903,
    lng: 133.649526,
    prefecture: "香川県",
    city: "丸亀市",
  },
  {
    uid: "KGW_TDT_TAKAMIJIMA",
    name: "高見島",
    kana: "たかみじま",
    en_name: "Takamijima",
    lat: 34.316717,
    lng: 133.673507,
    prefecture: "香川県",
    city: "多度津町",
  },
  {
    uid: "KGW_TDT_SANAGIJIMA",
    name: "佐柳島",
    kana: "さなぎじま",
    en_name: "Sanagijima",
    lat: 34.342534,
    lng: 133.625534,
    prefecture: "香川県",
    city: "多度津町",
  },
  {
    uid: "KGW_MTY_AWASHIMA",
    name: "粟島",
    kana: "あわしま",
    en_name: "Awashima",
    lat: 34.265934,
    lng: 133.625534,
    prefecture: "香川県",
    city: "三豊市",
  },
  {
    uid: "KGW_MTY_SHISHIJIMA",
    name: "志々島",
    kana: "ししじま",
    en_name: "Shishijima",
    lat: 34.271205,
    lng: 133.677104,
    prefecture: "香川県",
    city: "三豊市",
  },
  {
    uid: "KGW_KNO_IBUKIJIMA",
    name: "伊吹島",
    kana: "いぶきじま",
    en_name: "Ibukijima",
    lat: 34.129716,
    lng: 133.533052,
    prefecture: "香川県",
    city: "観音寺市",
  },
  {
    uid: "EHM_KMJ_UOSHIMA",
    name: "魚島",
    kana: "うおしま",
    en_name: "Uoshima",
    lat: 34.175934,
    lng: 133.317393,
    prefecture: "愛媛県",
    city: "上島町",
  },
  {
    uid: "EHM_KMJ_TAKAIKAMISHIMA",
    name: "高井神島",
    kana: "たかいかみしま",
    en_name: "Takaikamishima",
    lat: 34.188038,
    lng: 133.269074,
    prefecture: "愛媛県",
    city: "上島町",
  },
  {
    uid: "EHM_KMJ_YUGEJIMA",
    name: "弓削島",
    kana: "ゆげじま",
    en_name: "Yugejima",
    lat: 34.275896,
    lng: 133.218288,
    prefecture: "愛媛県",
    city: "上島町",
  },
  {
    uid: "EHM_KMJ_SASHIMA",
    name: "佐島",
    kana: "さしま",
    en_name: "Sashima",
    lat: 34.237573,
    lng: 133.186823,
    prefecture: "愛媛県",
    city: "上島町",
  },
  {
    uid: "EHM_KMJ_IKINAJIMA",
    name: "生名島",
    kana: "いきなじま",
    en_name: "Ikinajima",
    lat: 34.265654,
    lng: 133.179559,
    prefecture: "愛媛県",
    city: "上島町",
  },
  {
    uid: "EHM_KMJ_IWAGIJIMA",
    name: "岩城島",
    kana: "いわぎじま",
    en_name: "Iwagijima",
    lat: 34.260221,
    lng: 133.150494,
    prefecture: "愛媛県",
    city: "上島町",
  },
  {
    uid: "EHM_IMB_AKAHONEJIMA",
    name: "赤穂根島",
    kana: "あかほねじま",
    en_name: "Akahonejima",
    lat: 34.239081,
    lng: 133.162606,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_OHMISHIMA",
    name: "大三島",
    kana: "おおみしま",
    en_name: "Ohmishima",
    lat: 34.244366,
    lng: 133.009772,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_HAKATAJIMA",
    name: "伯方島",
    kana: "はかたじま",
    en_name: "Hakatajima",
    lat: 34.213042,
    lng: 133.092311,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_USHIMA",
    name: "鵜島",
    kana: "うしま",
    en_name: "Ushima",
    lat: 34.188544,
    lng: 133.088673,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 34.164611,
    lng: 133.068049,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_TSUSHIMA",
    name: "津島",
    kana: "つしま",
    en_name: "Tsushima",
    lat: 34.152026,
    lng: 132.997623,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_OKAMURAJIMA",
    name: "岡村島",
    kana: "おかむらじま",
    en_name: "Okamurajima",
    lat: 34.188892,
    lng: 132.880841,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_KOOGESHIMA",
    name: "小大下島",
    kana: "こおげしま",
    en_name: "Koogeshima",
    lat: 34.190182,
    lng: 132.90154,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_OHGESHIMA",
    name: "大下島",
    kana: "おおげしま",
    en_name: "Ohgeshima",
    lat: 34.192685,
    lng: 132.924665,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_KURUSHIMA",
    name: "来島",
    kana: "くるしま",
    en_name: "Kurushima",
    lat: 34.124709,
    lng: 132.979393,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_OSHIMA",
    name: "小島",
    kana: "おしま",
    en_name: "Oshima",
    lat: 34.124709,
    lng: 132.979393,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_UMASHIMA",
    name: "馬島",
    kana: "うましま",
    en_name: "Umashima",
    lat: 34.116955,
    lng: 132.99337,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_IMB_HIKJIMA",
    name: "比岐島",
    kana: "ひきじま",
    en_name: "Hikjima",
    lat: 34.058149,
    lng: 133.100194,
    prefecture: "愛媛県",
    city: "今治市",
  },
  {
    uid: "EHM_NIH_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 33.998386,
    lng: 133.360841,
    prefecture: "愛媛県",
    city: "新居浜市",
  },
  {
    uid: "EHM_MTY_AIJIMA",
    name: "安居島",
    kana: "あいじま",
    en_name: "Aijima",
    lat: 34.071243,
    lng: 132.705778,
    prefecture: "愛媛県",
    city: "松山市",
  },
  {
    uid: "EHM_MTY_GOGOSHIMA",
    name: "興居島",
    kana: "ごごしま",
    en_name: "Gogoshima",
    lat: 33.883688,
    lng: 132.673384,
    prefecture: "愛媛県",
    city: "松山市",
  },
  {
    uid: "EHM_MTY_TSURUSHIMA",
    name: "釣島",
    kana: "つるしま",
    en_name: "Tsurushima",
    lat: 33.890396,
    lng: 132.641582,
    prefecture: "愛媛県",
    city: "松山市",
  },
  {
    uid: "EHM_MTY_NOGUTSUNAJIMA",
    name: "野忽那島",
    kana: "のぐつなじま",
    en_name: "Nogutsunajima",
    lat: 33.969383,
    lng: 132.689278,
    prefecture: "愛媛県",
    city: "松山市",
  },
  {
    uid: "EHM_MTY_MUDUKIJIMA",
    name: "睦月島",
    kana: "むづきじま",
    en_name: "Mudukijima",
    lat: 33.964277,
    lng: 132.661154,
    prefecture: "愛媛県",
    city: "松山市",
  },
  {
    uid: "EHM_MTY_NAKAJIMA",
    name: "中島",
    kana: "なかじま",
    en_name: "Nakajima",
    lat: 33.975178,
    lng: 132.619553,
    prefecture: "愛媛県",
    city: "松山市",
  },
  {
    uid: "EHM_MTY_NUWAJIMA",
    name: "怒和島",
    kana: "ぬわじま",
    en_name: "Nuwajima",
    lat: 33.980306,
    lng: 132.548509,
    prefecture: "愛媛県",
    city: "松山市",
  },
  {
    uid: "EHM_MTY_TSUWAJISHIMA",
    name: "津和地島",
    kana: "つわじしま",
    en_name: "Tsuwajishima",
    lat: 33.9787,
    lng: 132.509271,
    prefecture: "愛媛県",
    city: "松山市",
  },
  {
    uid: "EHM_MTY_FUTAGAMIJIMA",
    name: "二神島",
    kana: "ふたがみじま",
    en_name: "Futagamijima",
    lat: 33.931694,
    lng: 132.533798,
    prefecture: "愛媛県",
    city: "松山市",
  },
  {
    uid: "EHM_OZ_AOSHIMA",
    name: "青島",
    kana: "あおしま",
    en_name: "Aoshima",
    lat: 33.735712,
    lng: 132.481663,
    prefecture: "愛媛県",
    city: "大洲市",
  },
  {
    uid: "EHM_YWT_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 33.38305,
    lng: 132.340948,
    prefecture: "愛媛県",
    city: "八幡浜市",
  },
  {
    uid: "EHM_UWJ_KUSHIMA",
    name: "九島",
    kana: "くしま",
    en_name: "Kushima",
    lat: 33.22912,
    lng: 132.523988,
    prefecture: "愛媛県",
    city: "宇和島市",
  },
  {
    uid: "EHM_UWJ_KASHIMA",
    name: "嘉島",
    kana: "かしま",
    en_name: "Kashima",
    lat: 33.228794,
    lng: 132.361249,
    prefecture: "愛媛県",
    city: "宇和島市",
  },
  {
    uid: "EHM_UWJ_TOJIMA",
    name: "戸島",
    kana: "とじま",
    en_name: "Tojima",
    lat: 33.212566,
    lng: 132.376624,
    prefecture: "愛媛県",
    city: "宇和島市",
  },
  {
    uid: "EHM_UWJ_HIBURIJIMA",
    name: "日振島",
    kana: "ひぶりじま",
    en_name: "Hiburijima",
    lat: 33.172509,
    lng: 132.281846,
    prefecture: "愛媛県",
    city: "宇和島市",
  },
  {
    uid: "EHM_UWJ_TAKEGASHIMA",
    name: "竹ケ島",
    kana: "たけがしま",
    en_name: "Takegashima",
    lat: 33.106552,
    lng: 132.412275,
    prefecture: "愛媛県",
    city: "宇和島市",
  },
  {
    uid: "KUC_SSK_NAKANOSHIMA",
    name: "中ノ島",
    kana: "なかのしま",
    en_name: "Nakanoshima",
    lat: 33.357419,
    lng: 133.30806,
    prefecture: "高知県",
    city: "須崎市",
  },
  {
    uid: "KUC_SSK_HESHIMA",
    name: "戸島",
    kana: "へしま",
    en_name: "Heshima",
    lat: 33.360934,
    lng: 133.300248,
    prefecture: "高知県",
    city: "須崎市",
  },
  {
    uid: "KUC_OTK_KASHIWAJIMA",
    name: "柏島",
    kana: "かしわじま",
    en_name: "Kashiwajima",
    lat: 32.765924,
    lng: 132.630568,
    prefecture: "高知県",
    city: "大月町",
  },
  {
    uid: "KUC_SKM_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 32.915811,
    lng: 132.696612,
    prefecture: "高知県",
    city: "宿毛市",
  },
  {
    uid: "KUC_SKM_UGURUSHIMA",
    name: "鵜来島",
    kana: "うぐるしま",
    en_name: "Ugurushima",
    lat: 32.802243,
    lng: 132.488413,
    prefecture: "高知県",
    city: "宿毛市",
  },
  {
    uid: "KUC_SKM_OKINOSHIMA",
    name: "沖の島",
    kana: "おきのしま",
    en_name: "Okinoshima",
    lat: 32.729018,
    lng: 132.555863,
    prefecture: "高知県",
    city: "宿毛市",
  },
  {
    uid: "FKO_KTK_UMASHIMA",
    name: "馬島",
    kana: "うましま",
    en_name: "Umashima",
    lat: 33.966832,
    lng: 130.85384,
    prefecture: "福岡県",
    city: "北九州市",
  },
  {
    uid: "FKO_KTK_AINOSHIMA",
    name: "藍島",
    kana: "あいのしま",
    en_name: "Ainoshima",
    lat: 33.995142,
    lng: 130.816529,
    prefecture: "福岡県",
    city: "北九州市",
  },
  {
    uid: "FKO_MNK_JINOSHIMA",
    name: "地島",
    kana: "じのしま",
    en_name: "Jinoshima",
    lat: 33.901572,
    lng: 130.499994,
    prefecture: "福岡県",
    city: "宗像市",
  },
  {
    uid: "FKO_MNK_AOSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Aoshima",
    lat: 33.900378,
    lng: 130.426164,
    prefecture: "福岡県",
    city: "宗像市",
  },
  {
    uid: "FKO_SNG_AINOSHIMA",
    name: "相島",
    kana: "あいのしま",
    en_name: "Ainoshima",
    lat: 33.76283,
    lng: 130.368817,
    prefecture: "福岡県",
    city: "新宮町",
  },
  {
    uid: "FKO_FKO_SHIKANOSHIMA",
    name: "志賀島",
    kana: "しかのしま",
    en_name: "Shikanoshima",
    lat: 33.666035,
    lng: 130.302786,
    prefecture: "福岡県",
    city: "福岡市",
  },
  {
    uid: "FKO_FKO_NOKONOSHIMA",
    name: "能古島",
    kana: "のこのしま",
    en_name: "Nokonoshima",
    lat: 33.622222,
    lng: 130.30631,
    prefecture: "福岡県",
    city: "福岡市",
  },
  {
    uid: "FKO_FKO_GENKAIJIMA",
    name: "玄界島",
    kana: "げんかいじま",
    en_name: "Genkaijima",
    lat: 33.689676,
    lng: 130.22968,
    prefecture: "福岡県",
    city: "福岡市",
  },
  {
    uid: "FKO_FKO_ORONOSHIMA",
    name: "小呂島",
    kana: "おろのしま",
    en_name: "Oronoshima",
    lat: 33.866969,
    lng: 130.036391,
    prefecture: "福岡県",
    city: "福岡市",
  },
  {
    uid: "FKO_ITS_HIMESHIMA",
    name: "姫島",
    kana: "ひめしま",
    en_name: "Himeshima",
    lat: 33.57086,
    lng: 130.049211,
    prefecture: "福岡県",
    city: "糸島市",
  },
  {
    uid: "SAG_KRT_TAKASHIMA",
    name: "高島",
    kana: "たかしま",
    en_name: "Takashima",
    lat: 33.47603,
    lng: 129.990217,
    prefecture: "佐賀県",
    city: "唐津市",
  },
  {
    uid: "SAG_KRT_KASHIWAJIMA",
    name: "神集島",
    kana: "かしわじま",
    en_name: "Kashiwajima",
    lat: 33.534753,
    lng: 129.972251,
    prefecture: "佐賀県",
    city: "唐津市",
  },
  {
    uid: "SAG_KRT_KABESHIMA",
    name: "加部島",
    kana: "かべしま",
    en_name: "Kabeshima",
    lat: 33.556956,
    lng: 129.881054,
    prefecture: "佐賀県",
    city: "唐津市",
  },
  {
    uid: "SAG_KRT_OGAWAJIMA",
    name: "小川島",
    kana: "おがわしま",
    en_name: "Ogawajima",
    lat: 33.598787,
    lng: 129.897762,
    prefecture: "佐賀県",
    city: "唐津市",
  },
  {
    uid: "SAG_KRT_KAKARASHIMA",
    name: "加唐島",
    kana: "かからしま",
    en_name: "Kakarashima",
    lat: 33.59982,
    lng: 129.85534,
    prefecture: "佐賀県",
    city: "唐津市",
  },
  {
    uid: "SAG_KRT_MATSUSHIMA",
    name: "松島",
    kana: "まつしま",
    en_name: "Matsushima",
    lat: 33.588966,
    lng: 129.843765,
    prefecture: "佐賀県",
    city: "唐津市",
  },
  {
    uid: "SAG_KRT_MADARASHIMA",
    name: "馬渡島",
    kana: "まだらしま",
    en_name: "Madarashima",
    lat: 33.57613,
    lng: 129.757527,
    prefecture: "佐賀県",
    city: "唐津市",
  },
  {
    uid: "SAG_KRT_MUKUSHIMA",
    name: "向島",
    kana: "むくしま",
    en_name: "Mukushima",
    lat: 33.49209,
    lng: 129.776845,
    prefecture: "佐賀県",
    city: "唐津市",
  },
  {
    uid: "NGS_TSM_TSUSHIMAJIMA",
    name: "対馬島",
    kana: "つしまじま",
    en_name: "Tsushimajima",
    lat: 34.285184,
    lng: 129.30399,
    prefecture: "長崎県",
    city: "対馬市",
  },
  {
    uid: "NGS_TSM_UNIJIMA",
    name: "海栗島",
    kana: "うにじま",
    en_name: "Unijima",
    lat: 34.706367,
    lng: 129.436885,
    prefecture: "長崎県",
    city: "対馬市",
  },
  {
    uid: "NGS_TSM_AKASHIMA",
    name: "赤島",
    kana: "あかしま",
    en_name: "Akashima",
    lat: 34.355901,
    lng: 129.407474,
    prefecture: "長崎県",
    city: "対馬市",
  },
  {
    uid: "NGS_TSM_TOMARISHIMA",
    name: "泊島",
    kana: "とまりしま",
    en_name: "Tomarishima",
    lat: 34.202781,
    lng: 129.28752,
    prefecture: "長崎県",
    city: "対馬市",
  },
  {
    uid: "NGS_TSM_OKINOSHIMA",
    name: "沖ノ島",
    kana: "おきのしま",
    en_name: "Okinoshima",
    lat: 34.342166,
    lng: 129.330891,
    prefecture: "長崎県",
    city: "対馬市",
  },
  {
    uid: "NGS_TSM_SHIMOYAMAJIMA",
    name: "島山島",
    kana: "しまやまじま",
    en_name: "Shimoyamajima",
    lat: 34.321248,
    lng: 129.321957,
    prefecture: "長崎県",
    city: "対馬市",
  },
  {
    uid: "NGS_IK_IKINOSHIMA",
    name: "壱岐島",
    kana: "いきのしま",
    en_name: "Ikinoshima",
    lat: 33.788435,
    lng: 129.713718,
    prefecture: "長崎県",
    city: "壱岐市",
  },
  {
    uid: "NGS_IK_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 33.739698,
    lng: 129.635041,
    prefecture: "長崎県",
    city: "壱岐市",
  },
  {
    uid: "NGS_IK_NAGASHIMA",
    name: "長島",
    kana: "ながしま",
    en_name: "Nagashima",
    lat: 33.727534,
    lng: 129.629878,
    prefecture: "長崎県",
    city: "壱岐市",
  },
  {
    uid: "NGS_IK_HARUSHIMA",
    name: "原島",
    kana: "はるしま",
    en_name: "Harushima",
    lat: 33.723321,
    lng: 129.648591,
    prefecture: "長崎県",
    city: "壱岐市",
  },
  {
    uid: "NGS_IK_WAKAMIYAJIMA",
    name: "若宮島",
    kana: "わかみやじま",
    en_name: "Wakamiyajima",
    lat: 33.865887,
    lng: 129.686644,
    prefecture: "長崎県",
    city: "壱岐市",
  },
  {
    uid: "NGS_MTU_FUKUSHIMA",
    name: "福島",
    kana: "ふくしま",
    en_name: "Fukushima",
    lat: 33.38399,
    lng: 129.821895,
    prefecture: "長崎県",
    city: "松浦市",
  },
  {
    uid: "NGS_MTU_TAKASHIMA",
    name: "鷹島",
    kana: "たかしま",
    en_name: "Takashima",
    lat: 33.445411,
    lng: 129.760103,
    prefecture: "長崎県",
    city: "松浦市",
  },
  {
    uid: "NGS_MTU_KUROSHIMA",
    name: "黒島",
    kana: "くろしま",
    en_name: "Kuroshima",
    lat: 33.450165,
    lng: 129.745475,
    prefecture: "長崎県",
    city: "松浦市",
  },
  {
    uid: "NGS_MTU_TOBISHIMA",
    name: "飛島",
    kana: "とびしま",
    en_name: "Tobishima",
    lat: 33.396711,
    lng: 129.77942,
    prefecture: "長崎県",
    city: "松浦市",
  },
  {
    uid: "NGS_MTU_AOSHIMA",
    name: "青島",
    kana: "あおしま",
    en_name: "Aoshima",
    lat: 33.409518,
    lng: 129.684065,
    prefecture: "長崎県",
    city: "松浦市",
  },
  {
    uid: "NGS_HRD_ADUCHIOHSHIMA",
    name: "的山大島",
    kana: "あづちおおしま",
    en_name: "AduchiOhshima",
    lat: 33.481611,
    lng: 129.547116,
    prefecture: "長崎県",
    city: "平戸市",
  },
  {
    uid: "NGS_HRD_TAKUSHIMA",
    name: "度島",
    kana: "たくしま",
    en_name: "Takushima",
    lat: 33.445517,
    lng: 129.530412,
    prefecture: "長崎県",
    city: "平戸市",
  },
  {
    uid: "NGS_HRD_HIRADOSHIMA",
    name: "平戸島",
    kana: "ひらどしま",
    en_name: "Hiradoshima",
    lat: 33.316132,
    lng: 129.465738,
    prefecture: "長崎県",
    city: "平戸市",
  },
  {
    uid: "NGS_HRD_TAKASHIMA",
    name: "高島",
    kana: "たかしま",
    en_name: "Takashima",
    lat: 33.186863,
    lng: 129.337079,
    prefecture: "長崎県",
    city: "平戸市",
  },
  {
    uid: "NGS_HRD_IKITSUKIJIMA",
    name: "生月島",
    kana: "いきつきじま",
    en_name: "Ikitsukijima",
    lat: 33.385862,
    lng: 129.419131,
    prefecture: "長崎県",
    city: "平戸市",
  },
  {
    uid: "NGS_SSB_MAEJIMA",
    name: "前島",
    kana: "まえじま",
    en_name: "Maejima",
    lat: 33.230438,
    lng: 129.560004,
    prefecture: "長崎県",
    city: "佐世保市",
  },
  {
    uid: "NGS_SSB_TOUDOMARIJIMA",
    name: "鼕泊島",
    kana: "とうどまりじま",
    en_name: "Toudomarijima",
    lat: 33.227534,
    lng: 129.554324,
    prefecture: "長崎県",
    city: "佐世保市",
  },
  {
    uid: "NGS_SSB_KUROSHIMA",
    name: "黒島",
    kana: "くろしま",
    en_name: "Kuroshima",
    lat: 33.142343,
    lng: 129.530412,
    prefecture: "長崎県",
    city: "佐世保市",
  },
  {
    uid: "NGS_SSB_TAKASHIMA",
    name: "高島",
    kana: "たかしま",
    en_name: "Takashima",
    lat: 33.177041,
    lng: 129.585978,
    prefecture: "長崎県",
    city: "佐世保市",
  },
  {
    uid: "NGS_SSB_HARIOJIMA",
    name: "針尾島",
    kana: "はりおじま",
    en_name: "Hariojima",
    lat: 33.114089,
    lng: 129.784842,
    prefecture: "長崎県",
    city: "佐世保市",
  },
  {
    uid: "NGS_SSB_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 33.072158,
    lng: 129.782639,
    prefecture: "長崎県",
    city: "佐世保市",
  },
  {
    uid: "NGS_SIK_TAKENOSHIMA",
    name: "竹ノ島",
    kana: "たけのしま",
    en_name: "Takenoshima",
    lat: 32.996328,
    lng: 129.743085,
    prefecture: "長崎県",
    city: "西海市",
  },
  {
    uid: "NGS_NGS_USEJIMA",
    name: "鵜瀬島",
    kana: "うせじま",
    en_name: "Usejima",
    lat: 32.913632,
    lng: 129.804767,
    prefecture: "長崎県",
    city: "長崎市",
  },
  {
    uid: "NGS_TGT_MAEJIMA",
    name: "前島",
    kana: "まえじま",
    en_name: "Maejima",
    lat: 32.869237,
    lng: 129.808384,
    prefecture: "長崎県",
    city: "時津町",
  },
  {
    uid: "NGS_ISH_KASHIMA",
    name: "鹿島",
    kana: "かしま",
    en_name: "Kashima",
    lat: 32.871877,
    lng: 129.927205,
    prefecture: "長崎県",
    city: "諫早市",
  },
  {
    uid: "NGS_SIK_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 33.046064,
    lng: 129.615679,
    prefecture: "長崎県",
    city: "西海市",
  },
  {
    uid: "NGS_SIK_TERASHIMA",
    name: "寺島",
    kana: "てらしま",
    en_name: "Terashima",
    lat: 33.036853,
    lng: 129.63375,
    prefecture: "長崎県",
    city: "西海市",
  },
  {
    uid: "NGS_SIK_KAKINOURASHIMA",
    name: "蛎浦島",
    kana: "かきのうらしま",
    en_name: "Kakinourashima",
    lat: 33.018421,
    lng: 129.582103,
    prefecture: "長崎県",
    city: "西海市",
  },
  {
    uid: "NGS_SIK_SAKITOJIMA",
    name: "崎戸島",
    kana: "さきとじま",
    en_name: "Sakitojima",
    lat: 33.004083,
    lng: 129.543328,
    prefecture: "長崎県",
    city: "西海市",
  },
  {
    uid: "NGS_SIK_ENOSHIMA",
    name: "江島",
    kana: "えのしま",
    en_name: "Enoshima",
    lat: 33.011007,
    lng: 129.349155,
    prefecture: "長崎県",
    city: "西海市",
  },
  {
    uid: "NGS_SIK_HIRASHIMA",
    name: "平島",
    kana: "ひらしま",
    en_name: "Hirashima",
    lat: 33.005571,
    lng: 129.242746,
    prefecture: "長崎県",
    city: "西海市",
  },
  {
    uid: "NGS_SIK_NAGUSHIJIMA",
    name: "南串島",
    kana: "なぐしじま",
    en_name: "Nagushijima",
    lat: 32.993735,
    lng: 129.650526,
    prefecture: "長崎県",
    city: "西海市",
  },
  {
    uid: "NGS_SIK_MATSUSHIMA",
    name: "松島",
    kana: "まつしま",
    en_name: "Matsushima",
    lat: 32.926549,
    lng: 129.605349,
    prefecture: "長崎県",
    city: "西海市",
  },
  {
    uid: "NGS_NGS_IKESHIMA",
    name: "池島",
    kana: "いけしま",
    en_name: "Ikeshima",
    lat: 32.885042,
    lng: 129.598893,
    prefecture: "長崎県",
    city: "長崎市",
  },
  {
    uid: "NGS_NGS_IOUJIMA",
    name: "伊王島",
    kana: "いおうじま",
    en_name: "Ioujima",
    lat: 32.697341,
    lng: 129.778132,
    prefecture: "長崎県",
    city: "長崎市",
  },
  {
    uid: "NGS_NGS_OKINOSHIMA",
    name: "沖之島",
    kana: "おきのしま",
    en_name: "Okinoshima",
    lat: 32.696118,
    lng: 129.779531,
    prefecture: "長崎県",
    city: "長崎市",
  },
  {
    uid: "NGS_NGS_TAKASHIMA",
    name: "高島",
    kana: "たかしま",
    en_name: "Takashima",
    lat: 32.659553,
    lng: 129.752375,
    prefecture: "長崎県",
    city: "長崎市",
  },
  {
    uid: "NGS_NGS_KABASHIMA",
    name: "樺島",
    kana: "かばしま",
    en_name: "Kabashima",
    lat: 32.561356,
    lng: 129.781988,
    prefecture: "長崎県",
    city: "長崎市",
  },
  {
    uid: "NGS_NGS_MAKISHIMA",
    name: "牧島",
    kana: "まきしま",
    en_name: "Makishima",
    lat: 32.755986,
    lng: 129.973534,
    prefecture: "長崎県",
    city: "長崎市",
  },
  {
    uid: "NGS_SSB_UKUJIMA",
    name: "宇久島",
    kana: "うくじま",
    en_name: "Ukujima",
    lat: 33.27212,
    lng: 129.102323,
    prefecture: "長崎県",
    city: "佐世保市",
  },
  {
    uid: "NGS_SSB_TERASHIMA",
    name: "寺島",
    kana: "てらしま",
    en_name: "Terashima",
    lat: 33.251041,
    lng: 129.066148,
    prefecture: "長崎県",
    city: "佐世保市",
  },
  {
    uid: "NGS_ODK_MUSHIMA",
    name: "六島",
    kana: "むしま",
    en_name: "Mushima",
    lat: 33.223122,
    lng: 129.134856,
    prefecture: "長崎県",
    city: "小値賀町",
  },
  {
    uid: "NGS_ODK_NOZAKIJIMA",
    name: "野崎島",
    kana: "のざきじま",
    en_name: "Nozakijima",
    lat: 33.19246,
    lng: 129.130953,
    prefecture: "長崎県",
    city: "小値賀町",
  },
  {
    uid: "NGS_ODK_NOUSHIMA",
    name: "納島",
    kana: "のうしま",
    en_name: "Noushima",
    lat: 33.222161,
    lng: 129.067168,
    prefecture: "長崎県",
    city: "小値賀町",
  },
  {
    uid: "NGS_ODK_ODIKAJIMA",
    name: "小値賀島",
    kana: "おぢかじま",
    en_name: "Odikajima",
    lat: 33.198619,
    lng: 129.063261,
    prefecture: "長崎県",
    city: "小値賀町",
  },
  {
    uid: "NGS_ODK_MADARASHIMA",
    name: "斑島",
    kana: "まだらしま",
    en_name: "Madarashima",
    lat: 33.206262,
    lng: 129.020266,
    prefecture: "長崎県",
    city: "小値賀町",
  },
  {
    uid: "NGS_ODK_KUROSHIMA",
    name: "黒島",
    kana: "くろしま",
    en_name: "Kuroshima",
    lat: 33.180576,
    lng: 129.06006,
    prefecture: "長崎県",
    city: "小値賀町",
  },
  {
    uid: "NGS_ODK_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 33.168151,
    lng: 129.020266,
    prefecture: "長崎県",
    city: "小値賀町",
  },
  {
    uid: "NGS_SNK_NAKADOORIJIMA",
    name: "中通島",
    kana: "なかどおりじま",
    en_name: "Nakadoorijima",
    lat: 32.955909,
    lng: 129.081493,
    prefecture: "長崎県",
    city: "新上五島町",
  },
  {
    uid: "NGS_SNK_KASHIRAGASHIMA",
    name: "頭ケ島",
    kana: "かしらがしま",
    en_name: "Kashiragashima",
    lat: 33.010918,
    lng: 129.186874,
    prefecture: "長崎県",
    city: "新上五島町",
  },
  {
    uid: "NGS_SNK_KIRINOKOJIMA",
    name: "桐ノ小島",
    kana: "きりのこじま",
    en_name: "Kirinokojima",
    lat: 32.924572,
    lng: 128.962893,
    prefecture: "長崎県",
    city: "新上五島町",
  },
  {
    uid: "NGS_SNK_WAKAMATSUJIMA",
    name: "若松島",
    kana: "わかまつじま",
    en_name: "Wakamatsujima",
    lat: 32.89027,
    lng: 129.013749,
    prefecture: "長崎県",
    city: "新上五島町",
  },
  {
    uid: "NGS_SNK_RYOUZEGAURASHIMA",
    name: "漁生浦島",
    kana: "りょうぜがうらしま",
    en_name: "Ryouzegaurashima",
    lat: 32.909884,
    lng: 128.964015,
    prefecture: "長崎県",
    city: "新上五島町",
  },
  {
    uid: "NGS_SNK_ARIFUKUJIMA",
    name: "有福島",
    kana: "ありふくじま",
    en_name: "Arifukujima",
    lat: 32.913871,
    lng: 128.943323,
    prefecture: "長崎県",
    city: "新上五島町",
  },
  {
    uid: "NGS_SNK_HINOSHIMA",
    name: "日島",
    kana: "ひのしま",
    en_name: "Hinoshima",
    lat: 32.924572,
    lng: 128.962893,
    prefecture: "長崎県",
    city: "新上五島町",
  },
  {
    uid: "NGS_GTU_NARUSHIMA",
    name: "奈留島",
    kana: "なるしま",
    en_name: "Narushima",
    lat: 32.840731,
    lng: 128.940713,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_MAESHIMA",
    name: "前島",
    kana: "まえしま",
    en_name: "Maeshima",
    lat: 32.80378,
    lng: 128.930094,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_HISAKAJIMA",
    name: "久賀島",
    kana: "ひさかじま",
    en_name: "Hisakajima",
    lat: 32.792011,
    lng: 128.851915,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_WARABIKOJIMA",
    name: "蕨小島",
    kana: "わらびこじま",
    en_name: "Warabikojima",
    lat: 32.828299,
    lng: 128.894044,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_KABASHIMA",
    name: "椛島",
    kana: "かばしま",
    en_name: "Kabashima",
    lat: 32.765551,
    lng: 128.995498,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_FUKUEJIMA",
    name: "福江島",
    kana: "ふくえじま",
    en_name: "Fukuejima",
    lat: 32.701861,
    lng: 128.75776,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_AKASHIMA",
    name: "赤島",
    kana: "あかしま",
    en_name: "Akashima",
    lat: 32.59392,
    lng: 128.918525,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_OUSHIMA",
    name: "黄島",
    kana: "おうしま",
    en_name: "Oushima",
    lat: 32.565375,
    lng: 128.901553,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_KUROSHIMA",
    name: "黒島",
    kana: "くろしま",
    en_name: "Kuroshima",
    lat: 32.599533,
    lng: 128.834978,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_SHIMAYAMAJIMA",
    name: "島山島",
    kana: "しまやまじま",
    en_name: "Shimayamajima",
    lat: 32.656897,
    lng: 128.624141,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "NGS_GTU_SAGANOSHIMA",
    name: "嵯峨島",
    kana: "さがのしま",
    en_name: "Saganoshima",
    lat: 32.724055,
    lng: 128.597909,
    prefecture: "長崎県",
    city: "五島市",
  },
  {
    uid: "KMM_UK_TOBASEJIMA",
    name: "戸馳島",
    kana: "とばせじま",
    en_name: "Tobasejima",
    lat: 32.594754,
    lng: 130.49745,
    prefecture: "熊本県",
    city: "宇城市",
  },
  {
    uid: "KMM_KMA_OHYANOJIMA",
    name: "大矢野島",
    kana: "おおやのじま",
    en_name: "Ohyanojima",
    lat: 32.584543,
    lng: 130.426164,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM_KMA_IWAJIMA",
    name: "維和島",
    kana: "いわじま",
    en_name: "Iwajima",
    lat: 32.568191,
    lng: 130.474546,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM_KMA_YAGYUUJIMA",
    name: "野牛島",
    kana: "やぎゅうじま",
    en_name: "Yagyuujima",
    lat: 32.567389,
    lng: 130.454713,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM_KMA_YUSHIMA",
    name: "湯島",
    kana: "ゆしま",
    en_name: "Yushima",
    lat: 32.603126,
    lng: 130.33821,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM_KMA_NOGAMAJIMA",
    name: "野釜島",
    kana: "のがまじま",
    en_name: "Nogamajima",
    lat: 32.586005,
    lng: 130.387133,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM_KMA_NAGAURAJIMA",
    name: "永浦島",
    kana: "ながうらじま",
    en_name: "Nagaurajima",
    lat: 32.541888,
    lng: 130.417247,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM_KMA_HIAIJIMA",
    name: "樋合島",
    kana: "ひあいじま",
    en_name: "Hiaijima",
    lat: 32.540696,
    lng: 130.401958,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM_KMA_MAEJIMA",
    name: "前島",
    kana: "まえじま",
    en_name: "Maejima",
    lat: 32.523764,
    lng: 130.427437,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM_KMA_NAKAJIMA",
    name: "中島",
    kana: "なかじま",
    en_name: "Nakajima",
    lat: 32.541137,
    lng: 130.413266,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM__AMAKUSAKAMSHIMA",
    name: "天草上島",
    kana: "あまくさかみしま",
    en_name: "Amakusakamshima",
    lat: 32.58735,
    lng: 130.430405,
    prefecture: "熊本県",
    city: "上天草市・天草市",
  },
  {
    uid: "KMM_KMA_HINOSHIMA",
    name: "樋島",
    kana: "ひのしま",
    en_name: "Hinoshima",
    lat: 32.38641,
    lng: 130.418521,
    prefecture: "熊本県",
    city: "上天草市",
  },
  {
    uid: "KMM_AMK_GOSYOURAJIMA",
    name: "御所浦島",
    kana: "ごしょうらじま",
    en_name: "Gosyourajima",
    lat: 32.331921,
    lng: 130.344588,
    prefecture: "熊本県",
    city: "天草市",
  },
  {
    uid: "KMM_AMK_MAKISHIMA",
    name: "牧島",
    kana: "まきしま",
    en_name: "Makishima",
    lat: 32.355488,
    lng: 130.318806,
    prefecture: "熊本県",
    city: "天草市",
  },
  {
    uid: "KMM_AMK_YOKOURAJIMA",
    name: "横浦島",
    kana: "よこうらじま",
    en_name: "Yokourajima",
    lat: 32.364818,
    lng: 130.348414,
    prefecture: "熊本県",
    city: "天草市",
  },
  {
    uid: "KMM__AMAKUSASHIMOSHIMA",
    name: "天草下島",
    kana: "あまくさしもしま",
    en_name: "Amakusashimoshima",
    lat: 32.409586,
    lng: 130.104306,
    prefecture: "熊本県",
    city: "天草市・苓北町",
  },
  {
    uid: "KMM_AMK_TUUJISHIMA",
    name: "通詞島",
    kana: "つうじしま",
    en_name: "Tuujishima",
    lat: 32.551257,
    lng: 130.11391,
    prefecture: "熊本県",
    city: "天草市",
  },
  {
    uid: "KMM_AMK_YOKOSHIMA",
    name: "横島",
    kana: "よこしま",
    en_name: "Yokoshima",
    lat: 32.369938,
    lng: 130.231559,
    prefecture: "熊本県",
    city: "天草市",
  },
  {
    uid: "KMM_AMK_GESUSHIMA",
    name: "下須島",
    kana: "げすしま",
    en_name: "Gesushima",
    lat: 32.178674,
    lng: 130.02998,
    prefecture: "熊本県",
    city: "天草市",
  },
  {
    uid: "OIT_HMJ_HIMESHIMA",
    name: "姫島",
    kana: "ひめしま",
    en_name: "Himeshima",
    lat: 33.726463,
    lng: 131.66602,
    prefecture: "大分県",
    city: "姫島村",
  },
  {
    uid: "OIT_USK_KUROSHIMA",
    name: "黒島",
    kana: "くろしま",
    en_name: "Kuroshima",
    lat: 33.177124,
    lng: 131.837411,
    prefecture: "大分県",
    city: "臼杵市",
  },
  {
    uid: "OIT_TKM_HOTOJIMA",
    name: "保戸島",
    kana: "ほとじま",
    en_name: "Hotojima",
    lat: 33.10468,
    lng: 132.010118,
    prefecture: "大分県",
    city: "津久見市",
  },
  {
    uid: "OIT_TKM_JIMUKUJIMA",
    name: "地無垢島",
    kana: "じむくしま",
    en_name: "Jimukujima",
    lat: 33.157362,
    lng: 131.976064,
    prefecture: "大分県",
    city: "津久見市",
  },
  {
    uid: "OIT_SIK_OHNYUJIMA",
    name: "大入島",
    kana: "おおにゅうじま",
    en_name: "Ohnyujima",
    lat: 32.988963,
    lng: 131.922128,
    prefecture: "大分県",
    city: "佐伯市",
  },
  {
    uid: "OIT_SIK_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 32.964608,
    lng: 132.074469,
    prefecture: "大分県",
    city: "佐伯市",
  },
  {
    uid: "OIT_SIK_YAKATAJIMA",
    name: "屋形島",
    kana: "やかたじま",
    en_name: "Yakatajima",
    lat: 32.77194,
    lng: 131.915926,
    prefecture: "大分県",
    city: "佐伯市",
  },
  {
    uid: "OIT_SIK_FUKASHIMA",
    name: "深島",
    kana: "ふかしま",
    en_name: "Fukashima",
    lat: 32.719039,
    lng: 131.925849,
    prefecture: "大分県",
    city: "佐伯市",
  },
  {
    uid: "MYZ_NBO_SHIMANOURASHIMA",
    name: "島野浦島",
    kana: "しまのうらしま",
    en_name: "Shimanourashima",
    lat: 32.662073,
    lng: 131.822812,
    prefecture: "宮崎県",
    city: "延岡市",
  },
  {
    uid: "MYZ_NCN_OHSHIMA",
    name: "大島",
    kana: "おおしま",
    en_name: "Ohshima",
    lat: 31.528139,
    lng: 131.408709,
    prefecture: "宮崎県",
    city: "日南市",
  },
  {
    uid: "MYZ_KSM_TSUKISHIMA",
    name: "築島",
    kana: "つきしま",
    en_name: "Tsukishima",
    lat: 31.477655,
    lng: 131.389925,
    prefecture: "宮崎県",
    city: "串間市",
  },
  {
    uid: "KGS_NGS_SHISHIJIMA",
    name: "獅子島",
    kana: "ししじま",
    en_name: "Shishijima",
    lat: 32.280052,
    lng: 130.232236,
    prefecture: "鹿児島県",
    city: "長島町",
  },
  {
    uid: "KGS_NGS_IKARAJIMA",
    name: "伊唐島",
    kana: "いからじま",
    en_name: "Ikarajima",
    lat: 32.224561,
    lng: 130.200279,
    prefecture: "鹿児島県",
    city: "長島町",
  },
  {
    uid: "KGS_NGS_SYOURAJIMA",
    name: "諸浦島",
    kana: "しょうらじま",
    en_name: "Syourajima",
    lat: 32.235556,
    lng: 130.168305,
    prefecture: "鹿児島県",
    city: "長島町",
  },
  {
    uid: "KGS_NGS_NAGASHIMA",
    name: "長島",
    kana: "ながしま",
    en_name: "Nagashima",
    lat: 32.199363,
    lng: 130.17699,
    prefecture: "鹿児島県",
    city: "長島町",
  },
  {
    uid: "KGS_IZM_KATSURAJIMA",
    name: "桂島",
    kana: "かつらじま",
    en_name: "Katsurajima",
    lat: 32.148972,
    lng: 130.266092,
    prefecture: "鹿児島県",
    city: "出水市",
  },
  {
    uid: "KGS_STM_KAMIKOSHIKISHIMA",
    name: "上甑島",
    kana: "かみこしきしま",
    en_name: "Kamikoshikishima",
    lat: 31.833476,
    lng: 129.888766,
    prefecture: "鹿児島県",
    city: "薩摩川内市",
  },
  {
    uid: "KGS_STM_NAKAKOSHIKISHIMA",
    name: "中甑島",
    kana: "なかこしきしま",
    en_name: "Nakakoshikishima",
    lat: 31.8006,
    lng: 129.829615,
    prefecture: "鹿児島県",
    city: "薩摩川内市",
  },
  {
    uid: "KGS_STM_SHIMOKOSIKISHIMA",
    name: "下甑島",
    kana: "しもこしきしま",
    en_name: "Shimokosikishima",
    lat: 31.703451,
    lng: 129.724029,
    prefecture: "鹿児島県",
    city: "薩摩川内市",
  },
  {
    uid: "KGS_KGS_SHINJIMA",
    name: "新島",
    kana: "しんじま",
    en_name: "Shinjima",
    lat: 31.617704,
    lng: 130.722827,
    prefecture: "鹿児島県",
    city: "鹿児島市",
  },
  {
    uid: "KGS__TANEGASHIMA",
    name: "種子島",
    kana: "たねがしま",
    en_name: "Tanegashima",
    lat: 30.73232,
    lng: 130.996955,
    prefecture: "鹿児島県",
    city: "西之表市・中種子町・南種子町",
  },
  {
    uid: "KGS_NSN_MAGESHIMA",
    name: "馬毛島",
    kana: "まげしま",
    en_name: "Mageshima",
    lat: 30.738748,
    lng: 130.852576,
    prefecture: "鹿児島県",
    city: "西之表市",
  },
  {
    uid: "KGS_YKS_YAKUSHIMA",
    name: "屋久島",
    kana: "やくしま",
    en_name: "Yakushima",
    lat: 30.344594,
    lng: 130.512714,
    prefecture: "鹿児島県",
    city: "屋久島町",
  },
  {
    uid: "KGS_YKS_KUCHINOERABUJIMA",
    name: "口永良部島",
    kana: "くちのえらぶじま",
    en_name: "Kuchinoerabujima",
    lat: 30.444711,
    lng: 130.216899,
    prefecture: "鹿児島県",
    city: "屋久島町",
  },
  {
    uid: "KGS_MSM_TAKESHIMA",
    name: "竹島",
    kana: "たけしま",
    en_name: "Takeshima",
    lat: 30.810191,
    lng: 130.428711,
    prefecture: "鹿児島県",
    city: "三島村",
  },
  {
    uid: "KGS_MSM_IOUJIMA",
    name: "硫黄島",
    kana: "いおうじま",
    en_name: "Ioujima",
    lat: 30.79217,
    lng: 130.303757,
    prefecture: "鹿児島県",
    city: "三島村",
  },
  {
    uid: "KGS_MSM_KUROSHIMA",
    name: "黒島",
    kana: "くろしま",
    en_name: "Kuroshima",
    lat: 30.832737,
    lng: 129.935018,
    prefecture: "鹿児島県",
    city: "三島村",
  },
  {
    uid: "KGS_TSM_KUCHINOSHIMA",
    name: "口之島",
    kana: "くちのしま",
    en_name: "Kuchinoshima",
    lat: 29.967374,
    lng: 129.924743,
    prefecture: "鹿児島県",
    city: "十島村",
  },
  {
    uid: "KGS_TSM_NAKANOSHIMA",
    name: "中之島",
    kana: "なかのしま",
    en_name: "Nakanoshima",
    lat: 29.856991,
    lng: 129.868198,
    prefecture: "鹿児島県",
    city: "十島村",
  },
  {
    uid: "KGS_TSM_SUWANOSEJIMA",
    name: "諏訪之瀬島",
    kana: "すわのせじま",
    en_name: "Suwanosejima",
    lat: 29.637537,
    lng: 129.708562,
    prefecture: "鹿児島県",
    city: "十島村",
  },
  {
    uid: "KGS_TSM_TAIRAJIMA",
    name: "平島",
    kana: "たいらじま",
    en_name: "Tairajima",
    lat: 29.688798,
    lng: 129.530412,
    prefecture: "鹿児島県",
    city: "十島村",
  },
  {
    uid: "KGS_TSM_AKUSEKIJIMA",
    name: "悪石島",
    kana: "あくせきじま",
    en_name: "Akusekijima",
    lat: 29.460113,
    lng: 129.605349,
    prefecture: "鹿児島県",
    city: "十島村",
  },
  {
    uid: "KGS_TSM_KODAKARAJIMA",
    name: "小宝島",
    kana: "こだからじま",
    en_name: "Kodakarajima",
    lat: 29.223296,
    lng: 129.324515,
    prefecture: "鹿児島県",
    city: "十島村",
  },
  {
    uid: "KGS_TSM_TAKARAJIMA",
    name: "宝島",
    kana: "たからじま",
    en_name: "Takarajima",
    lat: 29.148812,
    lng: 129.211568,
    prefecture: "鹿児島県",
    city: "十島村",
  },
  {
    uid: "KGS__AMAMIOHSHIMA",
    name: "奄美大島",
    kana: "あまみおおしま",
    en_name: "AmamiOhshima",
    lat: 28.37737,
    lng: 129.493807,
    prefecture: "鹿児島県",
    city: "奄美市・大和村・宇検村・龍郷町・瀬戸内町",
  },
  {
    uid: "KGS_STU_KAKEROMAJIMA",
    name: "加計呂麻島",
    kana: "かけろまじま",
    en_name: "Kakeromajima",
    lat: 28.116757,
    lng: 129.242746,
    prefecture: "鹿児島県",
    city: "瀬戸内町",
  },
  {
    uid: "KGS_STU_UKESHIMA",
    name: "請島",
    kana: "うけしま",
    en_name: "Ukeshima",
    lat: 28.024625,
    lng: 129.253135,
    prefecture: "鹿児島県",
    city: "瀬戸内町",
  },
  {
    uid: "KGS_STU_YOROSHIMA",
    name: "与路島",
    kana: "よろしま",
    en_name: "Yoroshima",
    lat: 28.039928,
    lng: 129.156969,
    prefecture: "鹿児島県",
    city: "瀬戸内町",
  },
  {
    uid: "KGS_KKI_KIKAIJIMA",
    name: "喜界島",
    kana: "きかいじま",
    en_name: "Kikaijima",
    lat: 28.321612,
    lng: 129.970967,
    prefecture: "鹿児島県",
    city: "喜界町",
  },
  {
    uid: "KGS__TOKUNOSHIMA",
    name: "徳之島",
    kana: "とくのしま",
    en_name: "Tokunoshima",
    lat: 27.726769,
    lng: 129.018809,
    prefecture: "鹿児島県",
    city: "徳之島町・伊仙町・天城町",
  },
  {
    uid: "KGS__OKINOERABUJIMA",
    name: "沖永良部島",
    kana: "おきのえらぶじま",
    en_name: "Okinoerabujima",
    lat: 27.391905,
    lng: 128.655616,
    prefecture: "鹿児島県",
    city: "和泊町・知名町",
  },
  {
    uid: "KGS_YRN_YORONJIMA",
    name: "与論島",
    kana: "よろんじま",
    en_name: "Yoronjima",
    lat: 27.042604,
    lng: 128.432407,
    prefecture: "鹿児島県",
    city: "与論町",
  },
  {
    uid: "OKN_MNM_MINAMIDAITOUJIMA",
    name: "南大東島",
    kana: "みなみだいとうじま",
    en_name: "Minamidaitoujima",
    lat: 25.848778,
    lng: 131.250732,
    prefecture: "沖縄県",
    city: "南大東村",
  },
  {
    uid: "OKN_KID_KITADAITOUJIMA",
    name: "北大東島",
    kana: "きただいとうじま",
    en_name: "Kitadaitoujima",
    lat: 25.94153,
    lng: 131.30595,
    prefecture: "沖縄県",
    city: "北大東村",
  },
  {
    uid: "OKN_IHY_IHEYAJIMA",
    name: "伊平屋島",
    kana: "いへやじま",
    en_name: "Iheyajima",
    lat: 27.050872,
    lng: 127.983755,
    prefecture: "沖縄県",
    city: "伊平屋村",
  },
  {
    uid: "OKN_IHY_NOHOJIMA",
    name: "野甫島",
    kana: "のほじま",
    en_name: "Nohojima",
    lat: 26.995061,
    lng: 127.91885,
    prefecture: "沖縄県",
    city: "伊平屋村",
  },
  {
    uid: "OKN_IZN_IZENAJIMA",
    name: "伊是名島",
    kana: "いぜなじま",
    en_name: "Izenajima",
    lat: 26.93416,
    lng: 127.941375,
    prefecture: "沖縄県",
    city: "伊是名村",
  },
  {
    uid: "OKN_OGM_MIYAGIJIMA",
    name: "宮城島",
    kana: "みやぎじま",
    en_name: "Miyagijima",
    lat: 26.667692,
    lng: 128.102342,
    prefecture: "沖縄県",
    city: "大宜味村",
  },
  {
    uid: "OKN_NKJ_KOURIJIMA",
    name: "古宇利島",
    kana: "こうりじま",
    en_name: "Kourijima",
    lat: 26.707298,
    lng: 128.018169,
    prefecture: "沖縄県",
    city: "今帰仁村",
  },
  {
    uid: "OKN_NG_YAGAJISHIMA",
    name: "屋我地島",
    kana: "やがじしま",
    en_name: "Yagajishima",
    lat: 26.670584,
    lng: 128.015522,
    prefecture: "沖縄県",
    city: "名護市",
  },
  {
    uid: "OKN_IE_IEJIMA",
    name: "伊江島",
    kana: "いえじま",
    en_name: "Iejima",
    lat: 26.720128,
    lng: 127.792839,
    prefecture: "沖縄県",
    city: "伊江村",
  },
  {
    uid: "OKN_MTB_SESOKOJIMA",
    name: "瀬底島",
    kana: "せそこじま",
    en_name: "Sesokojima",
    lat: 26.64515,
    lng: 127.864495,
    prefecture: "沖縄県",
    city: "本部町",
  },
  {
    uid: "OKN_MTB_MINNASHIMA",
    name: "水納島",
    kana: "みんなしま",
    en_name: "Minnashima",
    lat: 26.647906,
    lng: 127.817396,
    prefecture: "沖縄県",
    city: "本部町",
  },
  {
    uid: "OKN_URM_HENZAJIMA",
    name: "平安座島",
    kana: "へんざじま",
    en_name: "Henzajima",
    lat: 26.342154,
    lng: 127.958711,
    prefecture: "沖縄県",
    city: "うるま市",
  },
  {
    uid: "OKN_URM_MIYAGIJIMA",
    name: "宮城島",
    kana: "みやぎじま",
    en_name: "Miyagijima",
    lat: 26.369916,
    lng: 127.983755,
    prefecture: "沖縄県",
    city: "うるま市",
  },
  {
    uid: "OKN_URM_IKEIJIMA",
    name: "伊計島",
    kana: "いけいじま",
    en_name: "Ikeijima",
    lat: 26.393366,
    lng: 127.996993,
    prefecture: "沖縄県",
    city: "うるま市",
  },
  {
    uid: "OKN_URM_HAMAHIGAJIMA",
    name: "浜比嘉島",
    kana: "はまひがじま",
    en_name: "Hamahigajima",
    lat: 26.321324,
    lng: 127.958595,
    prefecture: "沖縄県",
    city: "うるま市",
  },
  {
    uid: "OKN_URM_TSUKENJIMA",
    name: "津堅島",
    kana: "つけんじま",
    en_name: "Tsukenjima",
    lat: 26.251838,
    lng: 127.94005,
    prefecture: "沖縄県",
    city: "うるま市",
  },
  {
    uid: "OKN_NNJ_KUDAKAJIMA",
    name: "久高島",
    kana: "くだかじま",
    en_name: "Kudakajima",
    lat: 26.161186,
    lng: 127.892341,
    prefecture: "沖縄県",
    city: "南城市",
  },
  {
    uid: "OKN_NNJ_OUJIMA",
    name: "奥武島",
    kana: "おうじま",
    en_name: "Oujima",
    lat: 26.129321,
    lng: 127.772455,
    prefecture: "沖縄県",
    city: "南城市",
  },
  {
    uid: "OKN_TMG_SENAGAJIMA",
    name: "瀬長島",
    kana: "せながじま",
    en_name: "Senagajima",
    lat: 26.175806,
    lng: 127.645511,
    prefecture: "沖縄県",
    city: "豊見城市",
  },
  {
    uid: "OKN_TNK_MAESHIMA",
    name: "前島",
    kana: "まえしま",
    en_name: "Maeshima",
    lat: 26.214016,
    lng: 127.448129,
    prefecture: "沖縄県",
    city: "渡嘉敷村",
  },
  {
    uid: "OKN_TNK_TOKASHIKIJIMA",
    name: "渡嘉敷島",
    kana: "とかしきじま",
    en_name: "Tokashikijima",
    lat: 26.200333,
    lng: 127.35601,
    prefecture: "沖縄県",
    city: "渡嘉敷村",
  },
  {
    uid: "OKN_ZMM_ZAMAMIJIMA",
    name: "座間味島",
    kana: "ざまみじま",
    en_name: "Zamamijima",
    lat: 26.233338,
    lng: 127.305226,
    prefecture: "沖縄県",
    city: "座間見村",
  },
  {
    uid: "OKN_ZMM_AKASHIMA",
    name: "阿嘉島",
    kana: "あかしま",
    en_name: "Akashima",
    lat: 26.20104,
    lng: 127.278483,
    prefecture: "沖縄県",
    city: "座間見村",
  },
  {
    uid: "OKN_ZMM_GERUMAJIMA",
    name: "慶留間島",
    kana: "げるまじま",
    en_name: "Gerumajima",
    lat: 26.182673,
    lng: 127.289182,
    prefecture: "沖縄県",
    city: "座間見村",
  },
  {
    uid: "OKN_AGN_AGUNIJIMA",
    name: "粟国島",
    kana: "あぐにじま",
    en_name: "Agunijima",
    lat: 26.589056,
    lng: 127.238351,
    prefecture: "沖縄県",
    city: "粟国村",
  },
  {
    uid: "OKN_TNK_TONAKIJIMA",
    name: "渡名喜島",
    kana: "となきじま",
    en_name: "Tonakijima",
    lat: 26.372126,
    lng: 127.141043,
    prefecture: "沖縄県",
    city: "渡名喜村",
  },
  {
    uid: "OKN_KMJ_KUMEJIMA",
    name: "久米島",
    kana: "くめじま",
    en_name: "Kumejima",
    lat: 26.350396,
    lng: 126.771177,
    prefecture: "沖縄県",
    city: "久米島町",
  },
  {
    uid: "OKN_KMJ_OUJIMA",
    name: "奥武島",
    kana: "おうじま",
    en_name: "Oujima",
    lat: 26.339507,
    lng: 126.829065,
    prefecture: "沖縄県",
    city: "久米島町",
  },
  {
    uid: "OKN_KMJ_OHHAJIMA",
    name: "オーハ島",
    kana: "おーはじま",
    en_name: "Ohhajima",
    lat: 26.341366,
    lng: 126.839829,
    prefecture: "沖縄県",
    city: "久米島町",
  },
  {
    uid: "OKN_MYK_MIYAKOJIMA",
    name: "宮古島",
    kana: "みやこじま",
    en_name: "Miyakojima",
    lat: 24.767367,
    lng: 125.324677,
    prefecture: "沖縄県",
    city: "宮古島市",
  },
  {
    uid: "OKN_MYK_IKEMAJIMA",
    name: "池間島",
    kana: "いけまじま",
    en_name: "Ikemajima",
    lat: 24.930518,
    lng: 125.245065,
    prefecture: "沖縄県",
    city: "宮古島市",
  },
  {
    uid: "OKN_MYK_OHGAMIJIMA",
    name: "大神島",
    kana: "おおがみじま",
    en_name: "Ohgamijima",
    lat: 24.916851,
    lng: 125.307526,
    prefecture: "沖縄県",
    city: "宮古島市",
  },
  {
    uid: "OKN_MYK_KURIMAJIMA",
    name: "来間島",
    kana: "くりまじま",
    en_name: "Kurimajima",
    lat: 24.722903,
    lng: 125.250558,
    prefecture: "沖縄県",
    city: "宮古島市",
  },
  {
    uid: "OKN_MYK_IRABUJIMA",
    name: "伊良部島",
    kana: "いらぶじま",
    en_name: "Irabujima",
    lat: 24.836724,
    lng: 125.18187,
    prefecture: "沖縄県",
    city: "宮古島市",
  },
  {
    uid: "OKN_MYK_SHIMOJISHIMA",
    name: "下地島",
    kana: "しもじしま",
    en_name: "Shimojishima",
    lat: 24.818321,
    lng: 125.154378,
    prefecture: "沖縄県",
    city: "宮古島市",
  },
  {
    uid: "OKN_TRM_TARAMAJIMA",
    name: "多良間島",
    kana: "たらまじま",
    en_name: "Taramajima",
    lat: 24.655531,
    lng: 124.696709,
    prefecture: "沖縄県",
    city: "多良間村",
  },
  {
    uid: "OKN_TRM_MINNASHIMA",
    name: "水納島",
    kana: "みんなしま",
    en_name: "Minnashima",
    lat: 24.757236,
    lng: 124.693945,
    prefecture: "沖縄県",
    city: "多良間村",
  },
  {
    uid: "OKN_ISG_ISHIGAKIJIMA",
    name: "石垣島",
    kana: "いしがきじま",
    en_name: "Ishigakijima",
    lat: 24.340686,
    lng: 124.155584,
    prefecture: "沖縄県",
    city: "石垣市",
  },
  {
    uid: "OKN_TKT_TAKETOMIJIMA",
    name: "竹富島",
    kana: "たけとみじま",
    en_name: "Taketomijima",
    lat: 24.326876,
    lng: 124.089197,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_TKT_KOHAMAJIMA",
    name: "小浜島",
    kana: "こはまじま",
    en_name: "Kohamajima",
    lat: 24.346608,
    lng: 123.980576,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_TKT_KAYAMAJIMA",
    name: "嘉弥真島",
    kana: "かやまじま",
    en_name: "Kayamajima",
    lat: 24.363552,
    lng: 123.999737,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_TKT_KUROSHIMA",
    name: "黒島",
    kana: "くろしま",
    en_name: "Kuroshima",
    lat: 24.233926,
    lng: 124.014011,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_TKT_ARAGUSUKUJIMA(KAMIJI)",
    name: "新城島上地",
    kana: "あらぐすくじま（かみじ）",
    en_name: "Aragusukujima(Kamiji)",
    lat: 24.23421,
    lng: 123.945734,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_TKT_ARAGUSUKUJIMA(SHIMOJI)",
    name: "新城島下地",
    kana: "あらぐすくじま（しもじ）",
    en_name: "Aragusukujima(Shimoji)",
    lat: 24.216649,
    lng: 123.927431,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_TKT_IRIOMOTEJIMA",
    name: "西表島",
    kana: "いりおもてじま",
    en_name: "Iriomotejima",
    lat: 24.330061,
    lng: 123.818798,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_TKT_YUBUJIMA",
    name: "由布島",
    kana: "ゆぶじま",
    en_name: "Yubujima",
    lat: 24.342479,
    lng: 123.93249,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_TKT_HATOMAJIMA",
    name: "鳩間島",
    kana: "はとまじま",
    en_name: "Hatomajima",
    lat: 24.472853,
    lng: 123.821589,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_TKT_HATERUMAJIMA",
    name: "波照間島",
    kana: "はてるまじま",
    en_name: "Haterumajima",
    lat: 24.063168,
    lng: 123.779705,
    prefecture: "沖縄県",
    city: "竹富町",
  },
  {
    uid: "OKN_YNG_YONAGUNIJIMA",
    name: "与那国島",
    kana: "よなぐにじま",
    en_name: "Yonagunijima",
    lat: 24.456032,
    lng: 122.994464,
    prefecture: "沖縄県",
    city: "与那国町",
  },
];
